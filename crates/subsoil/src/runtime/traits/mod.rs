...
impl IdentifyAccount for crate::core::ed25519::Public {
	type AccountId = Self;
...
impl IdentifyAccount for crate::core::sr25519::Public {
	type AccountId = Self;
	fn into_account(self) -> Self {
		self
	}
...
impl IdentifyAccount for crate::core::ecdsa::Public {
	type AccountId = Self;
	fn into_account(self) -> Self {
		self
	}
...
impl IdentifyAccount for crate::core::ecdsa_bls381::Public {
	type AccountId = Self;
	fn into_account(self) -> Self {
		self
	}
...
impl Verify for crate::core::ed25519::Signature {
	type Signer = crate::core::ed25519::Public;

	fn verify<L: Lazy<[u8]>>(&self, mut msg: L, signer: &crate::core::ed25519::Public) -> bool {
		crate::io::crypto::ed25519_verify(self, msg.get(), signer)
	}
}

impl Verify for crate::core::sr25519::Signature {
	type Signer = crate::core::sr25519::Public;

	fn verify<L: Lazy<[u8]>>(&self, mut msg: L, signer: &crate::core::sr25519::Public) -> bool {
		crate::io::crypto::sr25519_verify(self, msg.get(), signer)
	}
}

impl Verify for crate::core::ecdsa::Signature {
	type Signer = crate::core::ecdsa::Public;
	fn verify<L: Lazy<[u8]>>(&self, mut msg: L, signer: &crate::core::ecdsa::Public) -> bool {
		match crate::io::crypto::secp256k1_ecdsa_recover_compressed(
			self.as_ref(),
			&crate::io::hashing::blake2_256(msg.get()),
		) {
...
impl<
		S: Verify<
				Signer = <<T as AppCrypto>::Public as crate::application_crypto::AppPublic>::Generic,
			> + From<T>,
		T: crate::application_crypto::Wraps<Inner = S>
			+ crate::application_crypto::AppCrypto
			+ crate::application_crypto::AppSignature
			+ AsRef<S>
			+ AsMut<S>
			+ From<S>,
	> AppVerify for T
where
	...
	<<T as AppCrypto>::Public as crate::application_crypto::AppPublic>::Generic: IdentifyAccount<
		AccountId = <<T as AppCrypto>::Public as crate::application_crypto::AppPublic>::Generic,
	>,
{
	type AccountId = <T as AppCrypto>::Public;
	fn verify<L: Lazy<[u8]>>(&self, msg: L, signer: &<T as AppCrypto>::Public) -> bool {
		use crate::application_crypto::IsWrappedBy;
		let inner: &S = self.as_ref();
		let inner_pubkey =
			<<T as AppCrypto>::Public as crate::application_crypto::AppPublic>::Generic::from_ref(
				signer,
			);
...
where
	...
	crate::runtime::MultiAddress<AccountId, AccountIndex>: Codec + StaticTypeInfo,
{
	type Source = crate::runtime::MultiAddress<AccountId, AccountIndex>;
	type Target = AccountId;
	fn lookup(x: Self::Source) -> Result<Self::Target, LookupError> {
		match x {
			crate::runtime::MultiAddress::Id(i) => Ok(i),
			_ => Err(LookupError),
		}
	}
	fn unlookup(x: Self::Target) -> Self::Source {
		crate::runtime::MultiAddress::Id(x)
	}
}
...
macro_rules! morph_types {
	...
	(
		...
		$( #[doc = $doc] )*
		$vq struct $name < $($bound_id,)* > ( $crate::runtime::traits::PhantomData< ( $($bound_id,)* ) > ) ;
	};
	(
		@IMPL $name:ty : ( $( $bounds:tt )* ) ( $( $where:tt )* )
		= |$var:ident: $var_type:ty| -> $outcome:ty { $( $ex:expr )* }
	) => {
		impl<$($bounds)*> $crate::runtime::traits::Morph<$var_type> for $name $( $where )? {
	...
	(
		...
		impl<$($bounds)*> $crate::runtime::traits::TryMorph<$var_type> for $name $( $where )? {
			type Outcome = $outcome;
	...
	(
		...
		impl $crate::runtime::traits::Morph<$var_type> for $name $( $where )? {
			type Outcome = $outcome;
			fn morph($var: $var_type) -> Self::Outcome { $( $ex )* }
		}
	};
	(
		...
		impl $crate::runtime::traits::TryMorph<$var_type> for $name $( $where )? {
			type Outcome = $outcome;
			fn try_morph($var: $var_type) -> Result<Self::Outcome, ()> { $( $ex )* }
		}
	};
...
impl Hasher for BlakeTwo256 {
	type Out = crate::core::H256;
	type StdHasher = hash256_std_hasher::Hash256StdHasher;
	const LENGTH: usize = 32;

	fn hash(s: &[u8]) -> Self::Out {
		crate::io::hashing::blake2_256(s).into()
	}
}

impl Hash for BlakeTwo256 {
	type Output = crate::core::H256;

	fn ordered_trie_root(input: Vec<Vec<u8>>, version: StateVersion) -> Self::Output {
		crate::io::trie::blake2_256_ordered_root(input, version)
	}

	fn trie_root(input: Vec<(Vec<u8>, Vec<u8>)>, version: StateVersion) -> Self::Output {
		crate::io::trie::blake2_256_root(input, version)
	}
}
...
impl Hasher for Keccak256 {
	...
	fn hash(s: &[u8]) -> Self::Out {
		crate::io::hashing::keccak_256(s).into()
	}
}
...
impl Hash for Keccak256 {
	...
	fn ordered_trie_root(input: Vec<Vec<u8>>, version: StateVersion) -> Self::Output {
		crate::io::trie::keccak_256_ordered_root(input, version)
	}

	fn trie_root(input: Vec<(Vec<u8>, Vec<u8>)>, version: StateVersion) -> Self::Output {
		crate::io::trie::keccak_256_root(input, version)
	}
}
...
impl CheckEqual for crate::core::H256 {
	#[cfg(feature = "std")]
	fn check_equal(&self, other: &Self) {
...
crate::impl_maybe_marker_std_or_serde!(
...
pub trait RefundWeight {
	...
	fn refund(&mut self, weight: crate::weights::Weight);
}
...
impl RefundWeight for () {
	fn refund(&mut self, _weight: crate::weights::Weight) {}
}
...
pub trait Applyable: Sized + Send + Sync {
	...
	fn apply<V: ValidateUnsigned<Call = Self::Call>>(
		...
		info: &DispatchInfoOf<Self::Call>,
		len: usize,
	) -> crate::runtime::ApplyExtrinsicResultWithInfo<PostDispatchInfoOf<Self::Call>>;
}
...
macro_rules! impl_opaque_keys_inner {
	(
		...
		#[derive(
			Clone, PartialEq, Eq,
			$crate::runtime::codec::Encode,
			$crate::runtime::codec::Decode,
			$crate::runtime::codec::DecodeWithMemTracking,
			$crate::runtime::scale_info::TypeInfo,
			Debug,
		)]
		pub struct $name {
			$(
				$( #[ $inner_attr ] )*
				pub $field: <$type as $crate::runtime::BoundToRuntimeAppPublic>::Public,
			)*
		}
		...
		impl $name {
			...
			pub fn generate(
				owner: &[u8],
				seed: Option<$crate::std::vec::Vec<u8>>,
			) -> $crate::runtime::traits::GeneratedSessionKeys<
				Self,
				(
					$(
						<
							<$type as $crate::runtime::BoundToRuntimeAppPublic>::Public
								as $crate::runtime::RuntimeAppPublic
						>::ProofOfPossession
					),*
				)
			> {
				let mut keys = Self {
					$(
						$field: <
							<
								$type as $crate::runtime::BoundToRuntimeAppPublic
							>::Public as $crate::runtime::RuntimeAppPublic
			...
			pub fn into_raw_public_keys(
				self,
			) -> $crate::runtime::Vec<($crate::runtime::Vec<u8>, $crate::runtime::KeyTypeId)> {
				let mut keys = Vec::new();
				$(
					keys.push((
						$crate::runtime::RuntimeAppPublic::to_raw_vec(&self.$field),
						<
							<
								$type as $crate::runtime::BoundToRuntimeAppPublic
							>::Public as $crate::runtime::RuntimeAppPublic
			...
			pub fn decode_into_raw_public_keys(
				encoded: &[u8],
			) -> Option<$crate::runtime::Vec<($crate::runtime::Vec<u8>, $crate::runtime::KeyTypeId)>> {
				<Self as $crate::runtime::codec::Decode>::decode(&mut &encoded[..])
					.ok()
			...
			> {
				let res = ($(
					$crate::runtime::RuntimeAppPublic::generate_proof_of_possession(&mut self.$field, &owner).ok_or(())?
				),*);
		...
		impl $crate::runtime::traits::OpaqueKeys for $name {
			type KeyTypeIdProviders = ( $( $type, )* );

			fn key_ids() -> &'static [$crate::runtime::KeyTypeId] {
				&[
					$(
						<
							<
			...
			fn get_raw(&self, i: $crate::runtime::KeyTypeId) -> &[u8] {
				match i {
					$(
						i if i == <
							<
			...
			fn ownership_proof_is_valid(&self, owner: &[u8], proof: &[u8]) -> bool {
				...
				let Ok(proof) = <($(
						<
							...
							>::Public as $crate::runtime::RuntimeAppPublic
						>::ProofOfPossession
				),*) as $crate::runtime::codec::DecodeAll>::decode_all(&mut &proof[..]) else {
					return false
				};
				...
				$(
					let valid = $crate::runtime::RuntimeAppPublic::verify_proof_of_possession(&self.$field, &owner, &$field);
...
macro_rules! impl_opaque_keys {
	{
		...
		$crate::runtime::paste::paste! {
			use $crate::runtime::serde as [< __opaque_keys_serde_import__ $name >];

			$crate::impl_opaque_keys_inner! {
				$( #[ $attr ] )*
				#[derive($crate::runtime::serde::Serialize, $crate::runtime::serde::Deserialize)]
...
impl Printable for u64 {
	fn print(&self) {
		crate::io::misc::print_num(*self);
	}
}

impl Printable for &[u8] {
	fn print(&self) {
		crate::io::misc::print_hex(self);
	}
}
...
impl Printable for &str {
	fn print(&self) {
		crate::io::misc::print_utf8(self.as_bytes());
	}
}
...
impl Printable for crate::weights::Weight {
	fn print(&self) {
		self.ref_time().print()
	}
}
...
pub trait BlockIdTo<Block: self::Block> {
	...
	fn to_hash(
		&self,
		block_id: &crate::runtime::generic::BlockId<Block>,
...
mod tests {
	...
	impl crate::runtime::BoundToRuntimeAppPublic for Sr25519Key {
		type Public = crate::application_crypto::sr25519::AppPublic;
	}

	pub struct Ed25519Key;
	impl crate::runtime::BoundToRuntimeAppPublic for Ed25519Key {
		type Public = crate::application_crypto::ed25519::AppPublic;
	}

	pub struct EcdsaKey;
	impl crate::runtime::BoundToRuntimeAppPublic for EcdsaKey {
		type Public = crate::application_crypto::ecdsa::AppPublic;
	}
	...
	fn opaque_keys_ownership_proof_works() {
		let mut sr25519 = crate::core::sr25519::Pair::generate().0;
		let mut ed25519 = crate::core::ed25519::Pair::generate().0;
		let mut ecdsa = crate::core::ecdsa::Pair::generate().0;
		...
		let mut ext = crate::io::TestExternalities::default();
		ext.register_extension(crate::keystore::KeystoreExt(Arc::new(
			crate::keystore::testing::MemoryKeystore::new(),
		)));
