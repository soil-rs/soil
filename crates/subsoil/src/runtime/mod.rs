...
pub use crate::core::storage::StateVersion;
#[cfg(feature = "std")]
pub use crate::core::storage::{Storage, StorageChild};
...
pub use crate::application_crypto::{BoundToRuntimeAppPublic, RuntimeAppPublic};
...
#[cfg(feature = "std")]
pub use crate::core::{bounded_btree_map, bounded_vec};
...
pub use crate::arithmetic::helpers_128bit;
...
pub trait BuildStorage {
	...
	fn build_storage(&self) -> Result<crate::core::storage::Storage, String> {
...
impl BuildStorage for () {
	fn assimilate_storage(&self, _: &mut crate::core::storage::Storage) -> Result<(), String> {
...
impl traits::IdentifyAccount for MultiSigner {
	...
	fn into_account(self) -> AccountId32 {
		match self {
			...
			Self::Ecdsa(who) => crate::io::hashing::blake2_256(who.as_ref()).into(),
			Self::Eth(who) => {
				...
				// of an eth account.
				let eth_address = &crate::io::hashing::keccak_256(who.as_ref())[12..];
...
impl Verify for MultiSignature {
	...
	fn verify<L: Lazy<[u8]>>(&self, mut msg: L, signer: &AccountId32) -> bool {
		...
		match self {
			...
			Self::Ecdsa(sig) => {
				let m = crate::io::hashing::blake2_256(msg.get());
				crate::io::crypto::secp256k1_ecdsa_recover_compressed(sig.as_ref(), &m)
					.map_or(false, |pubkey| crate::io::hashing::blake2_256(&pubkey) == who)
			},
			Self::Eth(sig) => {
				let m = crate::io::hashing::keccak_256(msg.get());
				crate::io::crypto::secp256k1_ecdsa_recover_compressed(sig.as_ref(), &m)
					.map_or(false, |pubkey| {
...
impl From<traits::LookupError> for DispatchError {
	fn from(_: traits::LookupError) -> Self {
		Self::CannotLookup
	}
}

impl From<traits::BadOrigin> for DispatchError {
	fn from(_: traits::BadOrigin) -> Self {
		Self::BadOrigin
	}
}
...
// NOTE: assert_eq_error_rate macro is defined in crate::arithmetic
// (it is #[macro_export] so it lands at the crate root).
...
impl ::core::fmt::Debug for OpaqueExtrinsic {
	...
	fn fmt(&self, fmt: &mut ::core::fmt::Formatter) -> ::core::fmt::Result {
		write!(fmt, "{}", crate::core::hexdisplay::HexDisplay::from(&self.0.as_ref()))
	}
...
impl ::serde::Serialize for OpaqueExtrinsic {
	...
	where
		S: ::serde::Serializer,
	{
		codec::Encode::using_encoded(&self.0, |bytes| crate::core::bytes::serialize(bytes, seq))
	}
}
...
impl<'a> ::serde::Deserialize<'a> for OpaqueExtrinsic {
	...
	{
		let r = crate::core::bytes::deserialize(de)?;
		Decode::decode(&mut &r[..])
...
mod tests {
	...
	fn execute_and_generate_proof_works() {
		...
		let (_, proof) = ext.execute_and_prove(|| {
			crate::io::storage::get(b"a");
			crate::io::storage::get(b"b");
			crate::io::storage::get(b"v");
			crate::io::storage::get(b"d");
		});
		...
		let _ = ext.execute_and_prove(|| {
			crate::io::storage::set(b"a", &vec![1u8; 44]);
		});

		// ensure that these changes are propagated to the backend.

		ext.execute_with(|| {
			assert_eq!(crate::io::storage::get(b"a").unwrap(), vec![1u8; 44]);
			assert_eq!(crate::io::storage::get(b"b").unwrap(), vec![2u8; 33]);
		});
	}
}
