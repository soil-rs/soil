...
impl plant_session::SessionManager<u64> for TestSessionManager {
...
impl plant_session::historical::SessionManager<u64, u64> for TestSessionManager {
...
impl plant_session::Config for Runtime {
	...
	type SessionManager =
		plant_session::historical::NoteHistoricalRoot<Runtime, TestSessionManager>;
...
impl topsoil_support::traits::EstimateNextSessionRotation<u64> for TestNextSessionRotation {
	fn average_session_length() -> u64 {
		...
		mock.unwrap_or(plant_session::PeriodicSessions::<Period, Offset>::average_session_length())
	}

	fn estimate_current_session_progress(now: u64) -> (Option<Permill>, Weight) {
		let (estimate, weight) =
			plant_session::PeriodicSessions::<Period, Offset>::estimate_current_session_progress(
				now,
			);
	...
	fn estimate_next_session_rotation(now: u64) -> (Option<u64>, Weight) {
		plant_session::PeriodicSessions::<Period, Offset>::estimate_next_session_rotation(now)
	}
}
