...
//! listeners can hold onto a [`soil_client::client_api::UnpinHandle`] to keep a block pinned. Once the handle
...
mod tests {
	...
	fn pinning_worker_handles_base_case() {
		...
		let backend = Arc::new(soil_client::client_api::in_mem::Backend::<Block>::new());
