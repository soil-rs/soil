...
use plant_session::{historical::Pallet as Historical, Pallet as Session, *};
use plant_staking::{
...
pub struct Pallet<T: Config>(plant_session::Pallet<T>);
/// Configuration trait for the benchmarking of `plant-session`.
pub trait Config:
	plant_session::Config + plant_session::historical::Config + plant_staking::Config
{
...
impl<T: Config> OnInitialize<BlockNumberFor<T>> for Pallet<T> {
	fn on_initialize(n: BlockNumberFor<T>) -> topsoil_support::weights::Weight {
		plant_session::Pallet::<T>::on_initialize(n)
	}
}
