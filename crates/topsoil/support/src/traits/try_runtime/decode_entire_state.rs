...
fn decode_storage_info<V: Decode>(
	...
	let decode_key = |key: &[u8]| match subsoil::io::storage::get(key) {
		None => Ok(0),
		Some(bytes) => {
			let len = bytes.len();
	...
	loop {
		match next_key {
			Some(key) if key.starts_with(&info.prefix) => {
				...
				next_key = subsoil::io::storage::next_key(&key);
			},
			_ => break,
		}
	}
...
mod tests {
	...
	fn try_decode_entire_state_value_works() {
		subsoil::io::TestExternalities::new_empty().execute_with(|| {
			...
			subsoil::io::storage::set(&Value::hashed_key(), &[0u8, 1]);
	...
	fn try_decode_entire_state_map_works() {
		subsoil::io::TestExternalities::new_empty().execute_with(|| {
			...
			// two bytes, cannot be decoded into u32.
			subsoil::io::storage::set(&Map::hashed_key_for(2), &[0u8, 1]);
			...
			subsoil::io::storage::set(&Map::hashed_key_for(3), &[0u8, 1]);
			assert!(Map::try_decode_entire_state().is_err());
	...
	fn try_decode_entire_state_counted_map_works() {
		subsoil::io::TestExternalities::new_empty().execute_with(|| {
			...
			subsoil::io::storage::set(&CMap::hashed_key_for(2), &[0u8]);
	...
	fn try_decode_entire_state_double_works() {
		subsoil::io::TestExternalities::new_empty().execute_with(|| {
			...
			// two bytes, cannot be decoded into u32.
			subsoil::io::storage::set(&DMap::hashed_key_for(1, 1), &[0u8, 1]);
	...
	fn try_decode_entire_state_n_map_works() {
		subsoil::io::TestExternalities::new_empty().execute_with(|| {
			...
			subsoil::io::storage::set(&NMap::hashed_key_for((1, 1)), &[0u8, 1]);
	...
	fn try_decode_entire_state_counted_n_map_works() {
		subsoil::io::TestExternalities::new_empty().execute_with(|| {
			subsoil::io::TestExternalities::new_empty().execute_with(|| {
				...
				// two bytes, cannot be decoded into u128.
				subsoil::io::storage::set(&CNMap::hashed_key_for((1, 1)), &[0u8, 1]);
	...
	fn extra_bytes_are_rejected() {
		subsoil::io::TestExternalities::new_empty().execute_with(|| {
			...
			subsoil::io::storage::set(&Map::hashed_key_for(2), &[0u8, 1, 3, 4, 5, 6]);
			assert!(Map::try_decode_entire_state().is_err());
		})
	}
