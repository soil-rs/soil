...
fn process_message_error_reverts_storage_changes() {
	build_and_execute::<Test>(|| {
		assert!(!subsoil::io::storage::exists(b"key"), "Key should not exist");

		Callback::set(Box::new(|_, _| {
			subsoil::io::storage::set(b"key", b"value");
			Err(())
		}));
		...
		assert!(!subsoil::io::storage::exists(b"key"), "Key should have been rolled back");
	});
}
...
fn process_message_ok_false_keeps_storage_changes() {
	build_and_execute::<Test>(|| {
		...
		assert_eq!(subsoil::io::storage::exists(b"key"), true);
	});
}
