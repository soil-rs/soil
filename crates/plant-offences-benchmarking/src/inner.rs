...
use plant_babe::EquivocationOffence as BabeEquivocationOffence;
use plant_balances::Config as BalancesConfig;
use topsoil_benchmarking::v2::*;
use plant_grandpa::{
	EquivocationOffence as GrandpaEquivocationOffence, TimeSlot as GrandpaTimeSlot,
};
use plant_offences::{Config as OffencesConfig, Pallet as Offences};
use plant_session::{
...
pub trait Config:
	...
	+ plant_session_benchmarking::Config
	+ StakingConfig
	+ OffencesConfig
	+ HistoricalConfig
	+ BalancesConfig
...
fn make_offenders<T: Config>(
	...
	for i in 0..num_offenders {
		...
		// checks this.
		plant_session::Validators::<T>::mutate(|v| v.push(offender.controller.clone()));
...
where
	...
	<T as topsoil_system::Config>::RuntimeEvent: TryInto<plant_balances::Event<T>>,
	<T as topsoil_system::Config>::RuntimeEvent: TryInto<plant_offences::Event>,
	<T as topsoil_system::Config>::RuntimeEvent: TryInto<topsoil_system::Event<T>>,
{
	...
	assert_eq!(System::<T>::read_events_for_pallet::<plant_balances::Event<T>>().len(), 3);
	...
	// offence
	assert_eq!(System::<T>::read_events_for_pallet::<plant_offences::Event>().len(), 1);
