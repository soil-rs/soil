...
use soil_client::blockchain::Info;
use soil_client::client_api::{backend::Backend as BackendT, blockchain::HeaderBackend};
...
impl ChainInfoCmd {
	...
	pub fn run<B>(&self, config: &soil_service::Configuration) -> CliResult<()>
	where
		B: BlockT,
	{
		...
		let backend = soil_service::new_db_backend::<B>(db_config)?;
