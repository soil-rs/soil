...
impl<Block: BlockT> Blockchain<Block> {
	...
	pub fn set_head(&self, hash: Block::Hash) -> crate::blockchain::Result<()> {
		let header = self
			.header(hash)?
			.ok_or_else(|| crate::blockchain::Error::UnknownBlock(format!("{}", hash)))?;

		self.apply_head(&header)
	}

	fn apply_head(&self, header: &<Block as BlockT>::Header) -> crate::blockchain::Result<()> {
		...
		let best_tree_route = {
			...
			if &best_hash == header.parent_hash() {
				...
				let route = crate::blockchain::tree_route(self, best_hash, *header.parent_hash())?;
				Some(route)
			}
		};
	...
	fn append_justification(
		...
		if let Some(stored_justifications) = block_justifications {
			if !stored_justifications.append(justification) {
				return Err(crate::blockchain::Error::BadJustification(
...
impl<Block: BlockT> HeaderBackend<Block> for Blockchain<Block> {
	fn header(
		&self,
		hash: Block::Hash,
	) -> crate::blockchain::Result<Option<<Block as BlockT>::Header>> {
	...
	fn status(&self, hash: Block::Hash) -> crate::blockchain::Result<BlockStatus> {
	...
	fn number(&self, hash: Block::Hash) -> crate::blockchain::Result<Option<NumberFor<Block>>> {
...
impl<Block: BlockT> HeaderMetadata<Block> for Blockchain<Block> {
	...
	fn header_metadata(
		...
		self.header(hash)?
			...
			.ok_or_else(|| {
				crate::blockchain::Error::UnknownBlock(format!("header not found: {}", hash))
			})
	}
...
impl<Block: BlockT> blockchain::Backend<Block> for Blockchain<Block> {
	...
	fn last_finalized(&self) -> crate::blockchain::Result<Block::Hash> {
		Ok(self.storage.read().finalized_hash)
	}

	fn leaves(&self) -> crate::blockchain::Result<Vec<Block::Hash>> {
		Ok(self.storage.read().leaves.hashes())
	}

	fn children(&self, _parent_hash: Block::Hash) -> crate::blockchain::Result<Vec<Block::Hash>> {
		unimplemented!()
	}
...
impl<Block: BlockT> backend::AuxStore for Blockchain<Block> {
	...
	fn get_aux(&self, key: &[u8]) -> crate::blockchain::Result<Option<Vec<u8>>> {
...
impl<Block: BlockT> BlockImportOperation<Block> {
	...
	) -> crate::blockchain::Result<Block::Hash> {
...
impl<Block: BlockT> backend::BlockImportOperation<Block> for BlockImportOperation<Block> {
	...
	fn state(&self) -> crate::blockchain::Result<Option<&Self::State>> {
	...
	fn insert_aux<I>(&mut self, ops: I) -> crate::blockchain::Result<()>
	where
	...
	fn mark_head(&mut self, hash: Block::Hash) -> crate::blockchain::Result<()> {
...
impl<Block: BlockT> backend::Backend<Block> for Backend<Block> {
	...
	fn begin_operation(&self) -> crate::blockchain::Result<Self::BlockImportOperation> {
	...
	) -> crate::blockchain::Result<Self::State> {
	...
	) -> crate::blockchain::Result<(NumberFor<Block>, HashSet<Block::Hash>)> {
		Ok((Zero::zero(), HashSet::new()))
	}

	fn remove_leaf_block(&self, _hash: Block::Hash) -> crate::blockchain::Result<()> {
		Ok(())
	}
...
pub fn check_genesis_storage(storage: &Storage) -> crate::blockchain::Result<()> {
...
mod tests {
	...
	fn store_duplicate_justifications_is_forbidden() {
		...
		assert!(matches!(
			...
			Err(crate::blockchain::Error::BadJustification(_)),
		));
	}
}
