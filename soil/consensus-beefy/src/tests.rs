...
use crate::{
	...
	client::{
		beefy_block_import_and_links, finality_notification_transformer_future,
		gossip_protocol_name, wait_for_runtime_pallet, BeefyRPCLinks, BeefyVoterLinks,
		BeefyWorkerBuilder, UnpinnedFinalityNotification,
	},
};
use futures::{
	future,
...
async fn voter_init_setup(
	...
	finality: &mut futures::stream::Fuse<crate::client::FinalityNotifications<Block>>,
	api: &TestApi,
		...
		wait_for.push(Box::pin(stream.take(len).for_each(move |versioned_finality_proof| {
			...
			async move {
				let signed_commitment = match versioned_finality_proof {
					crate::VersionedFinalityProof::V1(sc) => sc,
				};
