...
pub use crate::communication::beefy_protocol_name::{
	gossip_protocol_name, justifications_protocol_name as justifs_protocol_name,
};
use soil_runtime::generic::OpaqueDigestItemId;

pub(crate) const LOG_TARGET: &str = "beefy";

pub(crate) const HEADER_SYNC_DELAY: Duration = Duration::from_secs(60);

pub(crate) type FinalityNotifications<Block> =
	...
	AuthorityId: AuthorityIdBound,
{
	...
	async fn init_state(
		...
		crate::aux_schema::write_current_version(backend.as_ref())?;
		crate::aux_schema::write_voter_state(backend.as_ref(), &state)?;
		Ok(state)
	}
...
/// blocks pinned.
pub(crate) struct UnpinnedFinalityNotification<B: Block> {
	...
	AuthorityId: AuthorityIdBound,
{
	...
	let gossip_validator =
		crate::communication::gossip::GossipValidator::new(known_peers.clone(), network.clone());
	...
	loop {
		...
		futures::select! {
			result = worker.run(&mut block_import_justif, &mut finality_notifications).fuse() => {
				match result {
					(crate::error::Error::ConsensusReset, reuse_comms) => {
						error!(target: LOG_TARGET, "🥩 Error: {:?}. Restarting voter.", crate::error::Error::ConsensusReset);
