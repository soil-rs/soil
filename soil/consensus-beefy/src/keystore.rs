...
use crate::{AuthorityIdBound, BeefyAuthorityId};

use crate::{error, client::LOG_TARGET};
...
impl<AuthorityId: AuthorityIdBound> BeefyKeystore<AuthorityId> {
	...
	/// Returns a vector of [`crate::crypto::Public`] keys which are currently
...
pub mod tests {
	...
	fn sign_error_for_ecdsa() {
		sign_error::<ecdsa_crypto::AuthorityId>(
			"crate::ecdsa_crypto::Public::try_sign_with_store() failed",
		);
	}
	...
	fn sign_error_for_ecdsa_n_bls() {
		sign_error::<ecdsa_bls_crypto::AuthorityId>(
			"crate::ecdsa_bls_crypto::Public::try_sign_with_store() failed",
		);
	}
