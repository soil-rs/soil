...
fn generate_host_functions_struct(
	...
	Ok(quote! {
		...
		#[cfg(not(substrate_runtime))]
		impl #crate_::soil_wasm_interface::HostFunctions for HostFunctions {
			fn host_functions() -> Vec<&'static dyn #crate_::soil_wasm_interface::Function> {
				let mut host_functions_list = Vec::new();
				#(#append_hf_bodies)*
				host_functions_list
			}

			#crate_::soil_wasm_interface::if_wasmtime_is_enabled! {
				fn register_static<T>(registry: &mut T) -> core::result::Result<(), T::Error>
					where T: #crate_::soil_wasm_interface::HostFunctionRegistry
				{
					#(#register_bodies)*
					Ok(())
				}
...
fn generate_host_function_implementation(
	...
	for (host_name, host_ty) in get_function_argument_names_and_types(&method.sig) {
		...
		convert_args_dynamic_ffi_to_static_ffi.push(quote! {
			...
			let #ffi_name: #ffi_ty = #crate_::soil_wasm_interface::TryFromValue::try_from_value(#ffi_name)
	...
	let convert_return_value_static_ffi_to_dynamic_ffi = match &method.sig.output {
		ReturnType::Type(_, _) => quote! {
			let __result__ = Ok(Some(#crate_::soil_wasm_interface::IntoValue::into_value(__result__)));
		},
		ReturnType::Default => quote! {
	...
	let implementation = quote! {
		...
		impl #struct_name {
			fn call(
				__function_context__: &mut dyn #crate_::soil_wasm_interface::FunctionContext,
				#(#ffi_args_prototype),*
		...
		#(#cfg_attrs)*
		#[cfg(not(substrate_runtime))]
		impl #crate_::soil_wasm_interface::Function for #struct_name {
			fn name(&self) -> &str {
				#name
			}

			fn signature(&self) -> #crate_::soil_wasm_interface::Signature {
				#signature
			}
			...
			fn execute(
				&self,
				__function_context__: &mut dyn #crate_::soil_wasm_interface::FunctionContext,
				args: &mut dyn Iterator<Item = #crate_::soil_wasm_interface::Value>,
			) -> ::core::result::Result<Option<#crate_::soil_wasm_interface::Value>, #crate_::alloc::string::String> {
	...
	let register_body = quote! {
		...
		registry.register_static(
			#crate_::soil_wasm_interface::Function::name(&#struct_name),
			|mut caller: #crate_::soil_wasm_interface::wasmtime::Caller<T::State>, #(#ffi_args_prototype),*|
				-> ::core::result::Result<#ffi_return_ty, #crate_::soil_wasm_interface::anyhow::Error>
			{
				T::with_function_context(caller, move |__function_context__| {
					let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
						#struct_name::call(
							__function_context__,
							#(#ffi_names,)*
						).map_err(#crate_::soil_wasm_interface::anyhow::Error::msg)
					}));
					match result {
						Ok(result) => result,
						Err(panic) => {
							...
							return Err(#crate_::soil_wasm_interface::anyhow::Error::msg(message));
						}
					}
				})
			}
	...
	let append_hf_body = quote! {
		#(#cfg_attrs)*
		host_functions_list.push(&#struct_name as &dyn #crate_::soil_wasm_interface::Function);
	};
...
fn generate_wasm_interface_signature_for_host_function(sig: &Signature) -> Result<TokenStream> {
	...
	let return_value = match &sig.output {
		ReturnType::Type(_, ty) => quote! {
			Some( <<#ty as #crate_::RIType>::FFIType as #crate_::soil_wasm_interface::IntoValue>::VALUE_TYPE )
		},
		ReturnType::Default => quote!(None),
	};
	let arg_types = get_function_argument_types(sig).map(|ty| {
		quote! {
			<<#ty as #crate_::RIType>::FFIType as #crate_::soil_wasm_interface::IntoValue>::VALUE_TYPE
		}
	});

	Ok(quote! {
		#crate_::soil_wasm_interface::Signature {
