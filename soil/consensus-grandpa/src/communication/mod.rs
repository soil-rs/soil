...
use crate::{
	client::{CommunicationIn, CommunicationOutH, LOG_TARGET},
	environment::HasVoted,
	CatchUp, Commit, CompactCommit, Error, Message, SignedMessage,
};
use gossip::{
...
use crate::{AuthorityId, AuthoritySignature, RoundNumber, SetId as SetIdNumber};
...
fn check_compact_commit<Block: BlockT>(
	...
	for (i, (precommit, (sig, id))) in msg.precommits.iter().zip(&msg.auth_data).enumerate() {
		...
		if !crate::check_message_signature_with_buffer(
