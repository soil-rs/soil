...
use crate::{AuthorityList, SetId, GRANDPA_ENGINE_ID};
use soil_runtime::{
	generic::BlockId,
		...
		for n in 1..=100 {
			...
			if n != 0 && n % 10 == 0 {
				...
				let digest = soil_runtime::generic::DigestItem::Consensus(
					crate::GRANDPA_ENGINE_ID,
					crate::ConsensusLog::ScheduledChange(
						crate::ScheduledChange { delay: 0u64, next_authorities },
					)
					.encode(),
				);
			...
			if let Some(new_authorities) = new_authorities {
				...
				for keyring in &current_authorities {
					...
					let encoded = crate::localized_payload(42, current_set_id, &msg);
