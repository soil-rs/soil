...
fn add_scheduled_change(builder: &mut impl BlockBuilderExt, change: ScheduledChange<BlockNumber>) {
	builder
		.push_deposit_log_digest_item(DigestItem::Consensus(
			GRANDPA_ENGINE_ID,
			crate::ConsensusLog::ScheduledChange(change).encode(),
		))
		.unwrap();
}
...
fn add_forced_change(
	...
	builder
		.push_deposit_log_digest_item(DigestItem::Consensus(
			GRANDPA_ENGINE_ID,
			crate::ConsensusLog::ForcedChange(median_last_finalized, change)
				.encode(),
		))
		.unwrap();
}
...
async fn justification_with_equivocation() {
	...
	let justification = {
		...
		let make_precommit = |target_hash, target_number, pair: &AuthorityPair| {
			...
			let encoded = crate::localized_payload(round, set_id, &msg);
...
async fn grandpa_environment_doesnt_send_equivocation_reports_for_itself() {
	...
	let equivocation_proof = crate::Equivocation::Prevote(equivocation.clone());
	...
	let equivocation_proof = crate::Equivocation::Prevote(equivocation);
