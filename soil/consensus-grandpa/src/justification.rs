...
pub struct GrandpaJustification<Block: BlockT> {
	...
	pub justification: crate::RawGrandpaJustification<Block::Header>,
	_block: PhantomData<Block>,
}

impl<Block: BlockT> From<crate::RawGrandpaJustification<Block::Header>>
	for GrandpaJustification<Block>
{
	fn from(justification: crate::RawGrandpaJustification<Block::Header>) -> Self {
		Self { justification, _block: Default::default() }
	}
}

impl<Block: BlockT> Into<crate::RawGrandpaJustification<Block::Header>>
	for GrandpaJustification<Block>
{
	fn into(self) -> crate::RawGrandpaJustification<Block::Header> {
		self.justification
	}
}
...
impl<Block: BlockT> GrandpaJustification<Block> {
		...
		C: HeaderBackend<Block>,
	{
		...
		Ok(crate::RawGrandpaJustification { round, commit, votes_ancestries }.into())
	}
		...
		NumberFor<Block>: finality_grandpa::BlockNumberOps,
	{
		let voters = VoterSet::new(authorities.iter().cloned())
			.ok_or(ClientError::Consensus(soil_consensus::Error::InvalidAuthoritiesSet))?;
		...
		NumberFor<Block>: finality_grandpa::BlockNumberOps,
	{
		...
		for signed in self.justification.commit.precommits.iter() {
			let signature_result = crate::check_message_signature_with_buffer(
