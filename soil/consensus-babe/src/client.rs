...
use crate::{inherents::BabeInherentData, SlotDuration};
...
pub use crate::aux_schema::load_block_weight as block_weight;
use soil_timestamp::Timestamp;


pub(crate) const LOG_TARGET: &str = "babe";

/// VRF context used for slots claiming lottery.
pub(crate) const AUTHORING_SCORE_VRF_CONTEXT: &[u8] = b"substrate-babe-vrf";

/// VRF output length for slots claiming lottery.
pub(crate) const AUTHORING_SCORE_LENGTH: usize = 16;

/// BABE epoch information
#[derive(Clone, Debug, PartialEq, Eq, Encode, Decode)]
pub struct Epoch(crate::Epoch);

impl Deref for Epoch {
	type Target = crate::Epoch;
...
impl From<crate::Epoch> for Epoch {
	fn from(epoch: crate::Epoch) -> Self {
		Epoch(epoch)
	}
}
...
pub(crate) fn babe_err<B: BlockT>(error: Error<B>) -> Error<B> {
	...
	Error: std::error::Error + Send + From<ConsensusError> + From<I::Error> + 'static,
{
	...
	let slot_worker = soil_consensus_slots::start_slot_worker(
		babe_link.config.slot_duration(),
		select_chain,
		soil_consensus_slots::SimpleSlotWorkerToSlotWorker(worker),
		sync_oracle,
		create_inherent_data_providers,
	);
...
fn aux_storage_cleanup<C: HeaderMetadata<Block> + HeaderBackend<Block>, Block: BlockT>(
	...
	hashes
		.into_iter()
		.map(|val| (crate::aux_schema::block_weight_key(val), None))
		.collect()
}
...
impl<B, C, E, I, Error, SO, L, BS> soil_consensus_slots::SimpleSlotWorker<B>
	...
	Error: std::error::Error + Send + From<ConsensusError> + From<I::Error> + 'static,
{
	...
	async fn claim_slot(
		...
		let s = crate::authorship::claim_slot(
			slot,
			self.epoch_changes
				.shared_data()
				.viable_epoch(epoch_descriptor, |slot| Epoch::genesis(&self.config, slot))?
	...
	Client::Api: BlockBuilderApi<Block> + BabeApi<Block>,
{
	async fn verify(
		...
		let (check_header, epoch_descriptor) = {
			...
			let v_params = crate::verification::VerificationParams {
			...
			(crate::verification::check_header::<Block>(v_params)?, epoch_descriptor)
		};
	...
	SC: soil_consensus::SelectChain<Block> + 'static,
{
	...
	async fn import_state(
		...
		// Reset block weight.
		crate::aux_schema::write_block_weight(hash, 0, |values| {
			block
				.auxiliary
		...
		let epoch_changes = if block.origin != BlockOrigin::WarpSync {
			...
			let (epoch_descriptor, first_in_epoch, parent_weight) = {
				let parent_weight = if *parent_header.number() == Zero::zero() {
					...
					crate::aux_schema::load_block_weight(&*self.client, parent_hash)
						.map_err(|e| ConsensusError::ClientImport(e.to_string()))?
						.ok_or_else(|| {
			...
			crate::aux_schema::write_block_weight(hash, total_weight, |values| {
				block
					.auxiliary
					.extend(values.iter().map(|(k, v)| (k.to_vec(), Some(v.to_vec()))))
			});
			...
			block.fork_choice = {
				...
				let last_best_weight = if &last_best == block.header.parent_hash() {
					...
					crate::aux_schema::load_block_weight(&*self.client, last_best)
						.map_err(|e| ConsensusError::ChainLookup(e.to_string()))?
						.ok_or_else(|| {
							ConsensusError::ChainLookup(
...
where
	Client: AuxStore
		...
		+ 'static,
{
	let epoch_changes = crate::aux_schema::load_epoch_changes::<Block, _>(&*client, &config)?;
	...
	Backend: BackendT<Block>,
{
	...
	let epoch_changes = crate::aux_schema::load_epoch_changes::<Block, Client>(&*client, &config)?;
	...
	for leaf in leaves {
		...
		loop {
			...
			if meta.number <= revert_up_to_number ||
				!weight_keys.insert(crate::aux_schema::block_weight_key(hash))
			{
	...
	crate::aux_schema::write_epoch_changes::<Block, _, _>(&epoch_changes, |values| {
