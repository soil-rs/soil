...
use crate::authorship::{calculate_primary_threshold, secondary_slot_author};
use super::client::{
	babe_err, find_pre_digest, Epoch, Error, AUTHORING_SCORE_LENGTH,
	AUTHORING_SCORE_VRF_CONTEXT, LOG_TARGET,
};
use soil_runtime::traits::Block as BlockT;
use log::{debug, trace};
use sc_consensus_epochs::Epoch as EpochT;
use soil_consensus_slots::CheckedHeader;
use crate::{
	digests::{
...
pub(crate) struct VerificationParams<'a, B: 'a + BlockT> {
	...
	/// work.
	pub(crate) pre_digest: Option<PreDigest>,
	/// The slot number of the current time.
	pub(crate) slot_now: Slot,
	/// Epoch descriptor of the epoch this block _should_ be under, if it's valid.
	pub(crate) epoch: &'a Epoch,
}
...
pub(crate) fn check_header<B: BlockT + Sized>(
...
pub(crate) struct VerifiedHeaderInfo {
	pub(crate) seal: DigestItem,
}
