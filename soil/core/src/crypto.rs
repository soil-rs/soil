	...
	pub fn soft<T: Encode>(index: T) -> Self {
		...
		index.using_encoded(|data| {
			if data.len() > JUNCTION_ID_LEN {
				cc.copy_from_slice(&soil_crypto_hashing::blake2_256(data));
			} else {
...
// Application-specific cryptographic traits.
//
// These traits define the interface for application-specific cryptographic
// types (key pairs, public keys, signatures) that wrap the generic crypto
// primitives with a specific `KeyTypeId`.
...
pub trait AppPublic:
	AppCrypto + Public + core::fmt::Debug + MaybeHash + codec::Codec
{
...
pub trait AppSignature:
	AppCrypto + Signature + Eq + PartialEq + core::fmt::Debug + Clone
{
