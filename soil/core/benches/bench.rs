...
use soil_core::crypto::Pair as _;
...
fn bench_ed25519(c: &mut Criterion) {
	...
	for &msg_size in &[32, 1024, 1024 * 1024] {
		...
		let key = soil_core::ed25519::Pair::generate().0;
	...
	for &msg_size in &[32, 1024, 1024 * 1024] {
		...
		group.bench_function(BenchmarkId::new("verifying", format!("{}", msg_size)), |b| {
			b.iter(|| soil_core::ed25519::Pair::verify(&sig, &msg, &public))
		});
	}
...
fn bench_sr25519(c: &mut Criterion) {
	...
	for &msg_size in &[32, 1024, 1024 * 1024] {
		let msg = (0..msg_size).map(|_| rand::random::<u8>()).collect::<Vec<_>>();
		let key = soil_core::sr25519::Pair::generate().0;
		group.bench_function(BenchmarkId::new("signing", format!("{}", msg_size)), |b| {
			b.iter(|| key.sign(&msg))
		});
	}
	...
	for &msg_size in &[32, 1024, 1024 * 1024] {
		...
		let public = key.public();
		group.bench_function(BenchmarkId::new("verifying", format!("{}", msg_size)), |b| {
			b.iter(|| soil_core::sr25519::Pair::verify(&sig, &msg, &public))
		});
	}
...
fn bench_ecdsa(c: &mut Criterion) {
	...
	for &msg_size in &[32, 1024, 1024 * 1024] {
		let msg = (0..msg_size).map(|_| rand::random::<u8>()).collect::<Vec<_>>();
		let key = soil_core::ecdsa::Pair::generate().0;
		group.bench_function(BenchmarkId::new("signing", format!("{}", msg_size)), |b| {
			b.iter(|| key.sign(&msg))
		});
	}
	...
	for &msg_size in &[32, 1024, 1024 * 1024] {
		...
		let public = key.public();
		group.bench_function(BenchmarkId::new("verifying", format!("{}", msg_size)), |b| {
			b.iter(|| soil_core::ecdsa::Pair::verify(&sig, &msg, &public))
		});
	}
