...
impl SlotTimestampProvider {
	...
	where
		...
		C::Api: BabeApi<B>,
	{
		let slot_duration = soil_consensus_babe::client::configuration(&*client)?.slot_duration();

		let time = Self::with_header(&client, slot_duration, |header| {
			let slot_number = *soil_consensus_babe::client::find_pre_digest::<B>(&header)
	...
	{
		let slot_duration = soil_consensus_aura::slot_duration(&*client)?;

		let time = Self::with_header(&client, slot_duration, |header| {
			let slot_number = *soil_consensus_aura::find_pre_digest::<B, AuthoritySignature>(&header)
				.map_err(|err| format!("{}", err))?;
			Ok(slot_number)
		})?;
