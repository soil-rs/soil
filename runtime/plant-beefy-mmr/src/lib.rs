...
where
	...
	T: plant_beefy::Config,
{
	fn on_new_root(root: &subsoil::consensus::beefy::MmrRootHash) {
		let digest = subsoil::runtime::generic::DigestItem::Consensus(
			subsoil::consensus::beefy::BEEFY_ENGINE_ID,
			codec::Encode::encode(&subsoil::consensus::beefy::ConsensusLog::<
...
pub mod pallet {
	...
	pub trait Config: plant_mmr::Config + plant_beefy::Config {
		...
		/// efficiency reasons.
		type BeefyAuthorityToMerkleLeaf: Convert<<Self as plant_beefy::Config>::BeefyId, Vec<u8>>;
...
impl<T> subsoil::consensus::beefy::OnNewValidatorSet<<T as plant_beefy::Config>::BeefyId>
	for Pallet<T>
where
	T: pallet::Config,
{
	...
	fn on_new_validator_set(
		current_set: &BeefyValidatorSet<<T as plant_beefy::Config>::BeefyId>,
		next_set: &BeefyValidatorSet<<T as plant_beefy::Config>::BeefyId>,
	) {
	...
	T: plant_mmr::Config<Hashing = subsoil::consensus::beefy::MmrHashing>,
{
	...
	fn extract_validation_context(header: HeaderFor<T>) -> Option<Self::ValidationContext> {
		...
		header.digest().convert_first(|l| {
			l.try_to(OpaqueDigestItemId::Consensus(&subsoil::consensus::beefy::BEEFY_ENGINE_ID))
				.and_then(|log: ConsensusLog<<T as plant_beefy::Config>::BeefyId>| match log {
...
impl<T: Config> Pallet<T> {
	...
	fn compute_authority_set(
		validator_set: &BeefyValidatorSet<<T as plant_beefy::Config>::BeefyId>,
