...
/// for epoch changes (e.g. plant-session) is used.
...
pub mod pallet {
	...
	pub trait Config: plant_timestamp::Config {
...
impl<T: Config> plant_session::ShouldEndSession<BlockNumberFor<T>> for Pallet<T> {
...
impl<T: Config> Pallet<T> {
	...
	/// manager logic like `plant-session`.
	///
	...
	T: plant_session::Config,
{
		...
		I: Iterator<Item = (&'a T::AccountId, AuthorityId)>,
	{
		...
		let session_index = <plant_session::Pallet<T>>::current_index();
