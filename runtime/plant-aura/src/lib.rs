...
//! ## Related Modules
//!
//! - [Timestamp](../plant_timestamp/index.html): The Timestamp module is used in Aura to track
...
/// [`plant_timestamp::Config::MinimumPeriod`] by multiplying it by two, to ensure
...
impl<T: plant_timestamp::Config> Get<T::Moment> for MinimumPeriodTimesTwo<T> {
	fn get() -> T::Moment {
		<T as plant_timestamp::Config>::MinimumPeriod::get().saturating_mul(2u32.into())
	}
}
...
pub mod pallet {
	...
	pub trait Config: plant_timestamp::Config + topsoil_core::system::Config {
		...
		#[pallet::constant]
		type SlotDuration: Get<<Self as plant_timestamp::Config>::Moment>;
	}
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		fn on_runtime_upgrade() -> Weight {
			use plant_timestamp::Pallet as Timestamp;
...
impl<T: Config> Pallet<T> {
	...
	pub fn do_try_state() -> Result<(), subsoil::runtime::TryRuntimeError> {
		...
		let timestamp = plant_timestamp::Pallet::<T>::get();
