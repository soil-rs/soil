	...
	Ok(quote! (
		...
		impl _feps::NposSolution for #ident {
			...
			fn corrupt(&mut self) {
				self.votes1.push(
					(
						_fepsp::subsoil::arithmetic::traits::Bounded::max_value(),
						_fepsp::subsoil::arithmetic::traits::Bounded::max_value()
					)
				)
			}
	...
	let check_minimum = (2..=count).map(|c| {
		...
		quote! {
			let filed_value = self.#filed
				.last()
				.map(|(x, ..)| voter_stake(x))
				.unwrap_or_else(|| _fepsp::subsoil::arithmetic::traits::Bounded::max_value());
				...
				for (voter_index, inners, t_last_idx) in self.#name {
					...
					let mut inners_parsed = inners
						...
						.map(|(ref t_idx, p)| {
							...
							sum = _fepsp::subsoil::arithmetic::traits::Saturating::saturating_add(sum, *p);
					...
					let p_last = _fepsp::subsoil::arithmetic::traits::Saturating::saturating_sub(
						#per_thing::one(),
						sum,
					);
