...
//! [`OnChargeAssetTransaction`] implementation analogous to `plant-transaction-payment`. The
//! included [`SwapAssetAdapter`] (implementing [`OnChargeAssetTransaction`]) determines the
//! fee amount by converting the fee calculated by `plant-transaction-payment` in the native
...
use plant_transaction_payment::{ChargeTransactionPayment, OnChargeTransaction};
...
///   [plant_transaction_payment::Config::OnChargeTransaction].
	...
	fn post_dispatch_details(
		...
		match initial_payment {
			InitialPayment::Native(already_withdrawn) => {
				...
				plant_transaction_payment::Pallet::<T>::deposit_fee_paid_event(
					who, actual_fee, tip,
				);
				Ok(unspent_weight)
			},
