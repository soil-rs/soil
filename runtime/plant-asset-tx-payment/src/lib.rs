...
//! [`OnChargeAssetTransaction`] implementation analogously to `plant-transaction-payment`. The
//! included [`FungiblesAdapter`] (implementing [`OnChargeAssetTransaction`]) determines the fee
//! amount by converting the fee calculated by `plant-transaction-payment` into the desired
//! asset.
//!
//! ## Integration
...
pub(crate) type OnChargeTransactionOf<T> =
	<T as plant_transaction_payment::Config>::OnChargeTransaction;
/// Balance type alias.
...
///
/// Wraps the transaction logic in [`plant_transaction_payment`] and extends it with assets.
	...
	<T::RuntimeCall as Dispatchable>::RuntimeOrigin: AsSystemOriginSigner<T::AccountId> + Clone,
{
	...
	> {
		use plant_transaction_payment::ChargeTransactionPayment;
		let Some(who) = origin.as_system_origin_signer() else {
			return Ok((ValidTransaction::default(), Val::NoCharge, origin));
		};
		// Non-mutating call of `compute_fee` to calculate the fee used in the transaction priority.
		let fee = plant_transaction_payment::Pallet::<T>::compute_fee(len as u32, info, self.tip);
	...
	fn post_dispatch_details(
		...
		match initial_payment {
			InitialPayment::Native(liquidity_info) => {
				...
				plant_transaction_payment::ChargeTransactionPayment::<T>::post_dispatch_details(
					plant_transaction_payment::Pre::Charge { tip, who, liquidity_info },
					info,
					&actual_post_info,
					len,
					result,
			...
			InitialPayment::Asset(already_withdrawn) => {
				...
				let mut actual_post_info = *post_info;
				actual_post_info.refund(unspent_weight);
				let actual_fee = plant_transaction_payment::Pallet::<T>::compute_actual_fee(
					len as u32,
					info,
					&actual_post_info,
					tip,
