...
fn new_block() -> Weight {
	let number = topsoil_core::system::Pallet::<Test>::block_number() + 1;
	let hash = H256::repeat_byte(number as u8);
	LeafDataTestValue::mutate(|r| r.a = number);

	topsoil_core::system::Pallet::<Test>::reset_events();
	topsoil_core::system::Pallet::<Test>::initialize(&number, &hash, &Default::default());
...
fn should_append_to_mmr_when_on_initialize_is_called() {
	...
	let (parent_b1, parent_b2) = ext.execute_with(|| {
		// when
		new_block();
		let parent_b1 = <topsoil_core::system::Pallet<Test>>::parent_hash();
		...
		// when
		new_block();
		let parent_b2 = <topsoil_core::system::Pallet<Test>>::parent_hash();
...
fn should_generate_proofs_correctly() {
	...
	ext.execute_with(|| {
		let best_block_number = topsoil_core::system::Pallet::<Test>::block_number();
...
fn generate_and_verify_batch_proof(
	...
	let max_block_number = ext.execute_with(|| topsoil_core::system::Pallet::<Test>::block_number());
...
fn should_verify_canonicalized() {
	...
	let block_hash_size: u64 = <Test as topsoil_core::system::Config>::BlockHashCount::get();
