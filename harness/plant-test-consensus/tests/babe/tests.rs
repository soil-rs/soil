...
use super::*;
use plant_babe::{Call, WeightInfo as _};
...
impl plant_babe::migrations::BabePalletPrefix for Test {
...
fn empty_randomness_is_correct() {
	let s = plant_babe::compute_randomness([0; RANDOMNESS_LENGTH], 0, std::iter::empty(), None);
	assert_eq!(s, EMPTY_RANDOMNESS);
}

#[test]
fn initial_values() {
	new_test_ext(4).execute_with(|| assert_eq!(plant_babe::Authorities::<Test>::get().len(), 4))
}
...
fn first_block_epoch_zero_start() {
	...
	ext.execute_with(|| {
		...
		assert_eq!(plant_babe::GenesisSlot::<Test>::get(), Slot::from(0));
		System::reset_events();
		...
		assert_eq!(plant_babe::GenesisSlot::<Test>::get(), genesis_slot);
		assert_eq!(CurrentSlot::<Test>::get(), genesis_slot);
		assert_eq!(plant_babe::EpochIndex::<Test>::get(), 0);

		Babe::on_finalize(1);
		let header = System::finalize();

		assert_eq!(plant_babe::AuthorVrfRandomness::<Test>::get(), Some(vrf_randomness));
		assert_eq!(plant_babe::SegmentIndex::<Test>::get(), 0);
		assert_eq!(plant_babe::UnderConstruction::<Test>::get(0), vec![vrf_randomness]);
		assert_eq!(plant_babe::Randomness::<Test>::get(), [0; 32]);
		assert_eq!(plant_babe::AuthorVrfRandomness::<Test>::get(), Some(vrf_randomness));
		assert_eq!(plant_babe::NextRandomness::<Test>::get(), [0; 32]);

		assert_eq!(header.digest.logs.len(), 2);
		assert_eq!(pre_digest.logs.len(), 1);
		assert_eq!(header.digest.logs[0], pre_digest.logs[0]);

		let consensus_log = subsoil::consensus::babe::ConsensusLog::NextEpochData(
			subsoil::consensus::babe::digests::NextEpochDescriptor {
				authorities: plant_babe::Authorities::<Test>::get().into_inner(),
				randomness: plant_babe::Randomness::<Test>::get(),
			},
		);
		let consensus_digest = DigestItem::Consensus(subsoil::consensus::babe::BABE_ENGINE_ID, consensus_log.encode());
...
fn current_slot_is_processed_on_initialization() {
	...
	ext.execute_with(|| {
		...
		assert!(plant_babe::Initialized::<Test>::get().is_none());

		// current slot is updated on initialization
		Babe::initialize(1);
		assert_eq!(CurrentSlot::<Test>::get(), genesis_slot);
		assert!(plant_babe::Initialized::<Test>::get().is_some());
		// but author vrf randomness isn't
		assert_eq!(plant_babe::AuthorVrfRandomness::<Test>::get(), None);
...
fn authority_index() {
	new_test_ext(4).execute_with(|| {
		assert_eq!(
			Babe::find_author((&[(subsoil::consensus::babe::BABE_ENGINE_ID, &[][..])]).into_iter().cloned()),
			None,
...
fn can_predict_next_epoch_change() {
	new_test_ext(1).execute_with(|| {
		...
		go_to_block(1, 6);
		assert_eq!(*plant_babe::GenesisSlot::<Test>::get(), 6);
		assert_eq!(*CurrentSlot::<Test>::get(), 6);
		assert_eq!(plant_babe::EpochIndex::<Test>::get(), 0);

		progress_to_block(5);

		assert_eq!(plant_babe::EpochIndex::<Test>::get(), 5 / 3);
...
fn can_estimate_current_epoch_progress() {
	new_test_ext(1).execute_with(|| {
		...
		for i in 1u64..4 {
			...
			if Babe::estimate_next_session_rotation(i).0.unwrap() - 1 == i {
				assert_eq!(
					...
					subsoil::runtime::Permill::from_percent(100)
				);
			} else {
				assert!(
					Babe::estimate_current_session_progress(i).0.unwrap()
		...
		assert_eq!(
			...
			subsoil::runtime::Permill::from_float(1.0 / 3.0),
		);
	})
}
...
fn can_enact_next_config() {
	new_test_ext(1).execute_with(|| {
		...
		plant_babe::EpochConfig::<Test>::put(current_config);
		plant_babe::NextEpochConfig::<Test>::put(next_config.clone());

		assert_eq!(plant_babe::NextEpochConfig::<Test>::get(), Some(next_config.clone()));

		Babe::plan_config_change(
			RuntimeOrigin::root(),
			subsoil::consensus::babe::digests::NextConfigDescriptor::V1 {
		...
		assert_eq!(plant_babe::EpochConfig::<Test>::get(), Some(next_config));
		assert_eq!(plant_babe::NextEpochConfig::<Test>::get(), Some(next_next_config.clone()));

		let consensus_log =
			subsoil::consensus::babe::ConsensusLog::NextConfigData(subsoil::consensus::babe::digests::NextConfigDescriptor::V1 {
				c: next_next_config.c,
				allowed_slots: next_next_config.allowed_slots,
			});
		let consensus_digest = DigestItem::Consensus(subsoil::consensus::babe::BABE_ENGINE_ID, consensus_log.encode());
...
fn only_root_can_enact_config_change() {
	...
	new_test_ext(1).execute_with(|| {
		let next_config =
			subsoil::consensus::babe::digests::NextConfigDescriptor::V1 { c: (1, 4), allowed_slots: AllowedSlots::PrimarySlots };
...
fn can_fetch_current_and_next_epoch_data() {
	new_test_ext(5).execute_with(|| {
		plant_babe::EpochConfig::<Test>::put(BabeEpochConfiguration {
			c: (1, 4),
			allowed_slots: subsoil::consensus::babe::AllowedSlots::PrimarySlots,
		});
...
fn tracks_block_numbers_when_current_and_previous_epoch_started() {
	new_test_ext(5).execute_with(|| {
		...
		let (last_epoch, current_epoch) = plant_babe::EpochStart::<Test>::get();
...
fn disabled_validators_cannot_author_blocks() {
	new_test_ext(4).execute_with(|| {
		...
		assert_eq!(plant_staking::CurrentEra::<Test>::get().unwrap(), 2);
...
fn report_equivocation_current_session_works() {
	...
	ext.execute_with(|| {
		...
		let authorities = plant_babe::Authorities::<Test>::get();
		...
		let key = (subsoil::consensus::babe::KEY_TYPE, &offending_authority_pair.public());
...
fn report_equivocation_invalid_key_owner_proof() {
	...
	ext.execute_with(|| {
		...
		assert_err!(
			...
			plant_babe::Error::<Test>::InvalidKeyOwnershipProof,
		);

		// it should fail as well if we create a key owner proof
		// for a different authority than the offender
		let key = (subsoil::consensus::babe::KEY_TYPE, &authorities[1].0);
		let key_owner_proof = Historical::prove(key).unwrap();
	...
	ext.execute_with(|| {
		...
		let assert_invalid_equivocation = |equivocation_proof| {
			assert_err!(
				...
				plant_babe::Error::<Test>::InvalidEquivocationProof,
			)
		};
...
fn report_equivocation_validate_unsigned_prevents_duplicates() {
	...
	ext.execute_with(|| {
		...
		assert_eq!(
			<Babe as subsoil::runtime::traits::ValidateUnsigned>::validate_unsigned(
		...
		assert_ok!(<Babe as subsoil::runtime::traits::ValidateUnsigned>::pre_dispatch(&inner));
		...
		assert_err!(
			<Babe as subsoil::runtime::traits::ValidateUnsigned>::pre_dispatch(&inner),
			InvalidTransaction::Stale,
		);
	});
}
...
fn report_equivocation_after_skipped_epochs_works() {
	...
	ext.execute_with(|| {
		...
		assert_eq!(plant_babe::EpochIndex::<Test>::get(), 10);
		assert_eq!(plant_babe::SkippedEpochs::<Test>::get(), vec![(10, 1)]);
...
fn add_epoch_configurations_migration_works() {
	...
	new_test_ext(1).execute_with(|| {
		let next_config_descriptor =
			subsoil::consensus::babe::digests::NextConfigDescriptor::V1 { c: (3, 4), allowed_slots: AllowedSlots::PrimarySlots };

		put_storage_value(b"Babe", b"NextEpochConfig", &[], Some(next_config_descriptor.clone()));

		assert!(get_storage_value::<Option<subsoil::consensus::babe::digests::NextConfigDescriptor>>(
			b"Babe",
			b"NextEpochConfig",
			&[],
		)
		...
		plant_babe::migrations::add_epoch_configuration::<Test>(current_epoch.clone());
		...
		assert_eq!(plant_babe::EpochConfig::<Test>::get(), Some(current_epoch));
		assert_eq!(plant_babe::PendingEpochConfigChange::<Test>::get(), Some(next_config_descriptor));
	});
}
...
fn skipping_over_epochs_works() {
	...
	ext.execute_with(|| {
		...
		assert_eq!(plant_babe::EpochIndex::<Test>::get(), 1);

		// genesis randomness is an array of zeros
		let randomness_for_epoch_2 = plant_babe::NextRandomness::<Test>::get();
		...
		assert_eq!(plant_babe::EpochIndex::<Test>::get(), 4);
		assert_eq!(plant_babe::Randomness::<Test>::get(), randomness_for_epoch_2);

		// after skipping epochs the information is registered on-chain so that
		// we can map epochs to sessions
		assert_eq!(plant_babe::SkippedEpochs::<Test>::get(), vec![(4, 2)]);
