...
impl plant_session::benchmarking::Config for Test {
	...
	fn setup_membership_proof_benchmark(n: u32) -> Result<MembershipBenchmarkSetup, &'static str> {
		...
		for who in plant_staking::testing_utils::create_validators::<Self>(n, 1000)? {
			...
			plant_session::Pallet::<Self>::set_keys(
		...
		plant_session::benchmarking::Pallet::<Self>::on_initialize(One::one());

		while Validators::<Self>::get().len() < n as usize {
			plant_session::Pallet::<Self>::rotate_session();
		}

		let key = first_key.ok_or("missing benchmark key")?;
		let proof = plant_session::historical::Pallet::<Self>::prove((key.0, key.1.clone())).ok_or("failed to prove")?;
