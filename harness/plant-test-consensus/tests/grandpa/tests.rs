...
use super::*;
use plant_grandpa::{Call, Event, WeightInfo as _};
use plant_grandpa::fg_primitives::ScheduledChange;
...
fn cannot_schedule_change_when_one_pending() {
	new_test_ext(vec![(1, 1), (2, 1), (3, 1)]).execute_with(|| {
		initialize_block(1, Default::default());
		Grandpa::schedule_change(to_authorities(vec![(4, 1), (5, 1), (6, 1)]), 1, None).unwrap();
		assert!(plant_grandpa::PendingChange::<Test>::exists());
		assert_noop!(
			Grandpa::schedule_change(to_authorities(vec![(5, 1)]), 1, None),
			plant_grandpa::Error::<Test>::ChangePending
		);
...
fn dispatch_forced_change() {
	new_test_ext(vec![(1, 1), (2, 1), (3, 1)]).execute_with(|| {
		...
		for i in 2..7 {
			...
			assert!(plant_grandpa::PendingChange::<Test>::get().unwrap().forced.is_some());
			assert_eq!(plant_grandpa::NextForced::<Test>::get(), Some(11));
			assert_noop!(
				Grandpa::schedule_change(to_authorities(vec![(5, 1)]), 1, None),
				plant_grandpa::Error::<Test>::ChangePending
			);
		...
		for i in 9..11 {
			...
			assert_noop!(
				...
				plant_grandpa::Error::<Test>::TooSoon
			);
			Grandpa::on_finalize(i);
			header = System::finalize();
		}

		{
			...
			assert_eq!(plant_grandpa::NextForced::<Test>::get(), Some(21));
...
fn schedule_pause_only_when_live() {
	new_test_ext(vec![(1, 1), (2, 1), (3, 1)]).execute_with(|| {
		...
		assert!(matches!(
			plant_grandpa::State::<Test>::get(),
			plant_grandpa::StoredState::PendingPause { scheduled_at: 1, delay: 1 }
		));
		...
		assert_noop!(Grandpa::schedule_pause(1), plant_grandpa::Error::<Test>::PauseFailed);

		Grandpa::on_finalize(2);
		let _ = System::finalize();

		// after finalizing block 2 the set should have switched to paused state
		assert!(matches!(plant_grandpa::State::<Test>::get(), plant_grandpa::StoredState::Paused));
	});
}
...
fn schedule_resume_only_when_paused() {
	new_test_ext(vec![(1, 1), (2, 1), (3, 1)]).execute_with(|| {
		...
		assert_noop!(Grandpa::schedule_resume(1), plant_grandpa::Error::<Test>::ResumeFailed);

		assert!(matches!(plant_grandpa::State::<Test>::get(), plant_grandpa::StoredState::Live));
...
fn time_slot_have_sane_ord() {
	...
	const FIXTURE: &[plant_grandpa::TimeSlot] = &[
		plant_grandpa::TimeSlot { set_id: 0, round: 0 },
		plant_grandpa::TimeSlot { set_id: 0, round: 1 },
		plant_grandpa::TimeSlot { set_id: 1, round: 0 },
		plant_grandpa::TimeSlot { set_id: 1, round: 1 },
		plant_grandpa::TimeSlot { set_id: 1, round: 2 },
	];
...
fn report_equivocation_current_set_works() {
	...
	new_test_ext_raw_authorities(authorities).execute_with(|| {
		...
		let set_id = plant_grandpa::CurrentSetId::<Test>::get();
		...
		let key_owner_proof =
			Historical::prove((subsoil::consensus::grandpa::KEY_TYPE, &equivocation_key)).unwrap();
...
fn report_equivocation_invalid_set_id() {
	...
	new_test_ext_raw_authorities(authorities).execute_with(|| {
		...
		assert_err!(
			...
			plant_grandpa::Error::<Test>::InvalidEquivocationProof,
		);
	});
}
...
fn report_equivocation_invalid_key_owner_proof() {
	...
	new_test_ext_raw_authorities(authorities).execute_with(|| {
		...
		let invalid_key_owner_proof =
			Historical::prove((subsoil::consensus::grandpa::KEY_TYPE, &invalid_owner_key)).unwrap();
		...
		assert_err!(
			...
			plant_grandpa::Error::<Test>::InvalidKeyOwnershipProof,
		);
	});
}
...
fn on_new_session_doesnt_start_new_set_if_schedule_change_failed() {
	new_test_ext(vec![(1, 1), (2, 1), (3, 1)]).execute_with(|| {
		assert_eq!(plant_grandpa::CurrentSetId::<Test>::get(), 0);

		// starting a new era should lead to a change in the session
		// validators and trigger a new set
		start_era(1);
		assert_eq!(plant_grandpa::CurrentSetId::<Test>::get(), 1);
		...
		start_era(3);
		assert_eq!(plant_grandpa::CurrentSetId::<Test>::get(), 2);
		...
		plant_grandpa::NextForced::<Test>::put(1000);
		plant_grandpa::Stalled::<Test>::put((30, 1));
...
fn cleans_up_old_set_id_session_mappings() {
	new_test_ext(vec![(1, 1), (2, 1), (3, 1)]).execute_with(|| {
		...
		for i in 1..=max_set_id_session_entries {
			assert!(plant_grandpa::SetIdSession::<Test>::get(i as u64).is_some());
		}
		...
		for i in 1..=max_set_id_session_entries {
			assert!(plant_grandpa::SetIdSession::<Test>::get(i as u64).is_none());
		}
	});
}
...
fn always_schedules_a_change_on_new_session_when_stalled() {
	new_test_ext(vec![(1, 1), (2, 1), (3, 1)]).execute_with(|| {
		...
		assert!(plant_grandpa::PendingChange::<Test>::get().is_none());
		assert_eq!(plant_grandpa::CurrentSetId::<Test>::get(), 1);
		...
		plant_grandpa::Stalled::<Test>::put((10, 1));
		Grandpa::on_new_session(false, std::iter::empty(), std::iter::empty());

		assert!(plant_grandpa::PendingChange::<Test>::get().is_some());
		assert!(plant_grandpa::PendingChange::<Test>::get().unwrap().forced.is_some());
		assert_eq!(plant_grandpa::CurrentSetId::<Test>::get(), 2);
	});
}
...
fn stalled_state_is_kept_on_schedule_change_failure() {
	new_test_ext(vec![(1, 1), (2, 1), (3, 1)]).execute_with(|| {
		...
		assert!(plant_grandpa::PendingChange::<Test>::exists(), "PendingChange should exist");

		// Inject the Stalled state to simulate a network partition.
		plant_grandpa::Stalled::<Test>::put((10, 1));
		assert!(plant_grandpa::Stalled::<Test>::exists(), "Stalled state should exist");
