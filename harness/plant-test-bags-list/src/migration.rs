...
pub async fn execute<Runtime, Block>(
	...
	Runtime: RuntimeT<plant_bags_list::Instance1>,
	...
	Block::Header: DeserializeOwned,
{
	let mut ext = Builder::<Block>::new()
		.mode(Mode::Online(OnlineConfig {
			...
			pallets: vec![plant_staking::Pallet::<Runtime>::name().to_string()],
			..Default::default()
		}))
		.build()
		.await
	...
	ext.execute_with(|| {
		...
		let moved = <Runtime as plant_staking::Config>::VoterList::unsafe_regenerate(
			...
			Box::new(|x| Some(plant_staking::Pallet::<Runtime>::weight_of(x))),
		);
		log::info!(target: LOG_TARGET, "Moved {} nominators", moved);

		let voter_list_len = <Runtime as plant_staking::Config>::VoterList::iter().count() as u32;
