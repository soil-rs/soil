...
fn set_balance_handles_total_issuance() {
	ExtBuilder::default().build_and_execute_with(|| {
		let old_total_issuance = plant_balances::TotalIssuance::<Test>::get();
		assert_ok!(Balances::force_set_balance(RuntimeOrigin::root(), 1337, 69));
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), old_total_issuance + 69);
			...
			assert_ok!(<Test as Config>::AccountStore::try_mutate_exists(
				...
				|a| -> DispatchResult {
					*a = Some(AccountData {
						...
						reserved: 5,
						frozen: Zero::zero(),
						flags: plant_balances::ExtraFlags::old_logic(),
					});
					Ok(())
				}
			));
...
fn force_adjust_total_issuance_example() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		let old_ti = plant_balances::TotalIssuance::<Test>::get();
		...
		let new_ti = plant_balances::TotalIssuance::<Test>::get();
...
fn force_adjust_total_issuance_works() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		let ti = plant_balances::TotalIssuance::<Test>::get();

		// Increase works:
		assert_ok!(Balances::force_adjust_total_issuance(RawOrigin::Root.into(), Inc, 32));
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), ti + 32);
...
fn force_adjust_total_issuance_saturates() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), max);
...
fn force_adjust_total_issuance_rejects_more_than_inactive() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), 64);
		...
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), 16);
		...
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), 26);
...
fn burn_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let init_issuance = plant_balances::TotalIssuance::<Test>::get();
		...
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), init_issuance - burn_amount_1);
