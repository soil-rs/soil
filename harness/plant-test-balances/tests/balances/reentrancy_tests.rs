...
fn transfer_dust_removal_tst1_should_work() {
	ExtBuilder::default()
		...
		.build_and_execute_with(|| {
			...
			System::assert_has_event(RuntimeEvent::Balances(plant_balances::Event::Transfer {
				from: 2,
				to: 3,
				amount: 450,
			}));
			System::assert_has_event(RuntimeEvent::Balances(plant_balances::Event::DustLost {
				account: 2,
				amount: 50,
			}));
...
fn emit_events_with_no_existential_deposit_suicide_with_dust() {
	ExtBuilder::default().existential_deposit(2).build_and_execute_with(|| {
		...
		assert_eq!(
			events(),
			[
				RuntimeEvent::Balances(plant_balances::Event::Withdraw { who: 1, amount: 98 }),
				RuntimeEvent::Balances(plant_balances::Event::BurnedDebt { amount: 98 })
			]
		);
		...
		assert_eq!(
			...
			[
				...
				RuntimeEvent::Balances(plant_balances::Event::DustLost { account: 1, amount: 1 }),
				RuntimeEvent::Balances(plant_balances::Event::BurnedDebt { amount: 1 }),
				RuntimeEvent::Balances(plant_balances::Event::Withdraw { who: 1, amount: 1 }),
				RuntimeEvent::Balances(plant_balances::Event::BurnedDebt { amount: 1 })
			]
		);
	});
