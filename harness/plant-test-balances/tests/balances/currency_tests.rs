...
use super::*;
use plant_balances::{Event, NegativeImbalance};
...
pub const CALL: &<Test as topsoil_system::Config>::RuntimeCall =
	&RuntimeCall::Balances(plant_balances::Call::transfer_allow_death { dest: 0, value: 0 });
...
fn lock_should_work_reserve() {
	ExtBuilder::default()
		...
		.monied(true)
		.build_and_execute_with(|| {
			plant_transaction_payment::NextFeeMultiplier::<Test>::put(
...
fn reward_should_work() {
	ExtBuilder::default().monied(true).build_and_execute_with(|| {
		...
		assert_eq!(
			events(),
			[
				RuntimeEvent::Balances(plant_balances::Event::Deposit { who: 1, amount: 10 }),
				RuntimeEvent::Balances(plant_balances::Event::Issued { amount: 10 }),
			]
		);
		assert_eq!(Balances::total_balance(&1), 20);
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), 120);
	});
}
...
fn balance_works() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		System::assert_has_event(RuntimeEvent::Balances(plant_balances::Event::Deposit {
			who: 1,
			amount: 42,
		}));
...
fn slashing_balance_should_work() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		assert_eq!(Balances::reserved_balance(1), 69);
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), 70);
	});
}
...
fn withdrawing_balance_should_work() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		System::assert_has_event(RuntimeEvent::Balances(plant_balances::Event::Withdraw {
			who: 2,
			amount: 11,
		}));
		assert_eq!(Balances::free_balance(2), 100);
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), 100);
	});
}
...
fn slashing_incomplete_balance_should_work() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), 22);
	});
}
...
fn slashing_incomplete_reserved_balance_should_work() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		assert_eq!(Balances::reserved_balance(1), 0);
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), 69);
	});
}
...
fn repatriating_reserved_balance_should_work() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::Balances(plant_balances::Event::ReserveRepatriated {
			from: 1,
			to: 2,
			amount: 41,
...
fn account_create_on_free_too_low() {
	ExtBuilder::default().existential_deposit(100).build_and_execute_with(|| {
		...
		let _ = Balances::deposit_creating(&2, 50);
		assert_eq!(Balances::free_balance(2), 0);
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), 0);
	})
}
...
fn account_removal_on_free_too_low() {
	ExtBuilder::default().existential_deposit(100).build_and_execute_with(|| {
		...
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), 220);
		...
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), 130);
	});
}
...
fn burn_must_work() {
	ExtBuilder::default().monied(true).build_and_execute_with(|| {
		...
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), init_total_issuance - 10);
		drop(imbalance);
		assert_eq!(plant_balances::TotalIssuance::<Test>::get(), init_total_issuance);
	});
}
...
fn cannot_set_genesis_value_below_ed() {
	...
	let _ = plant_balances::GenesisConfig::<Test> { balances: vec![(1, 10)], ..Default::default() }
...
fn emit_events_with_reserve_and_unreserve() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::Balances(plant_balances::Event::Reserved {
			who: 1,
			amount: 10,
		}));
		...
		System::assert_last_event(RuntimeEvent::Balances(plant_balances::Event::Unreserved {
			who: 1,
			amount: 5,
		}));
...
fn emit_events_with_changing_locks() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		assert_eq!(events(), [RuntimeEvent::Balances(plant_balances::Event::Locked { who: 1, amount: 10 })]);

		// Locks = [10] --> [15]
		Balances::set_lock(*b"LOCK_000", &1, 15, WithdrawReasons::TRANSFER);
		assert_eq!(events(), [RuntimeEvent::Balances(plant_balances::Event::Locked { who: 1, amount: 5 })]);
		...
		for event in events() {
			match event {
				RuntimeEvent::Balances(plant_balances::Event::Locked { .. }) => {
					assert!(false, "unexpected lock event")
				},
				RuntimeEvent::Balances(plant_balances::Event::Unlocked { .. }) => {
		...
		assert_eq!(
			events(),
			[RuntimeEvent::Balances(plant_balances::Event::Unlocked { who: 1, amount: 3 })]
		);
		...
		assert_eq!(
			events(),
			[RuntimeEvent::Balances(plant_balances::Event::Unlocked { who: 1, amount: 2 })]
		);
		...
		assert_eq!(
			events(),
			[RuntimeEvent::Balances(plant_balances::Event::Unlocked { who: 1, amount: 15 })]
		);
	});
}
	...
	ExtBuilder::default().existential_deposit(100).build_and_execute_with(|| {
		...
		assert_eq!(
			...
			[
				...
				RuntimeEvent::Balances(plant_balances::Event::Endowed { account: 1, free_balance: 100 }),
				RuntimeEvent::Balances(plant_balances::Event::Issued { amount: 100 }),
				RuntimeEvent::Balances(plant_balances::Event::BalanceSet { who: 1, free: 100 }),
			]
		);
		...
		assert_eq!(
			...
			[
				...
				RuntimeEvent::Balances(plant_balances::Event::DustLost { account: 1, amount: 99 }),
				RuntimeEvent::Balances(plant_balances::Event::BurnedDebt { amount: 99 }),
				RuntimeEvent::Balances(plant_balances::Event::Slashed { who: 1, amount: 1 }),
				RuntimeEvent::Balances(plant_balances::Event::Rescinded { amount: 1 }),
			]
		);
	});
}
...
fn emit_events_with_no_existential_deposit_suicide() {
	ExtBuilder::default().existential_deposit(1).build_and_execute_with(|| {
		...
		assert_eq!(
			...
			[
				RuntimeEvent::System(system::Event::KilledAccount { account: 1 }),
				RuntimeEvent::Balances(plant_balances::Event::Slashed { who: 1, amount: 100 }),
				RuntimeEvent::Balances(plant_balances::Event::Rescinded { amount: 100 }),
			]
		);
	});
}
...
fn slash_full_works() {
	ExtBuilder::default().existential_deposit(100).build_and_execute_with(|| {
		...
		assert!(!System::account_exists(&1));
		System::assert_has_event(RuntimeEvent::Balances(plant_balances::Event::Slashed {
			who: 1,
			amount: 1000,
		}));
	});
