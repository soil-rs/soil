...
fn unbalanced_trait_set_balance_works() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::Balances(plant_balances::Event::Released {
			reason: TestId::Foo,
			who: 1337,
			amount: 60,
		}));
	...
	ExtBuilder::default()
		...
		.build_and_execute_with(|| {
			...
			assert_eq!(
				events(),
				[
					RuntimeEvent::Balances(plant_balances::Event::Frozen { who: 1, amount: 10 }),
					RuntimeEvent::Balances(plant_balances::Event::Held {
						reason: TestId::Foo,
						who: 1,
						amount: 9
					}),
					RuntimeEvent::Balances(plant_balances::Event::TransferOnHold {
						reason: TestId::Foo,
						source: 1,
						dest: 2,
						amount: 1
		...
		.build_and_execute_with(|| {
			...
			assert_eq!(
				...
				[
					...
					RuntimeEvent::Balances(plant_balances::Event::TransferAndHold {
						reason: TestId::Foo,
						source: 1,
						dest: 2,
						transferred: 1
...
fn burn_held() {
	ExtBuilder::default()
		...
		.build_and_execute_with(|| {
			...
			System::assert_last_event(RuntimeEvent::Balances(plant_balances::Event::BurnedHeld {
				reason: TestId::Foo,
				who: account,
				amount: 4,
			}));
...
fn negative_imbalance_drop_handled_correctly() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::Balances(plant_balances::Event::BurnedDebt {
...
fn positive_imbalance_drop_handled_correctly() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		System::assert_last_event(RuntimeEvent::Balances(plant_balances::Event::MintedCredit {
...
fn emit_events_with_changing_freezes() {
	ExtBuilder::default().build_and_execute_with(|| {
		...
		// Freeze = [] --> [10]
		assert_ok!(Balances::set_freeze(&TestId::Foo, &1, 10));
		assert_eq!(events(), [RuntimeEvent::Balances(plant_balances::Event::Frozen { who: 1, amount: 10 })]);

		// Freeze = [10] --> [15]
		assert_ok!(Balances::set_freeze(&TestId::Foo, &1, 15));
		assert_eq!(events(), [RuntimeEvent::Balances(plant_balances::Event::Frozen { who: 1, amount: 5 })]);
		...
		for event in events() {
			match event {
				RuntimeEvent::Balances(plant_balances::Event::Frozen { .. }) => {
					assert!(false, "unexpected freeze event")
				},
				RuntimeEvent::Balances(plant_balances::Event::Thawed { .. }) => {
		...
		assert_eq!(events(), [RuntimeEvent::Balances(plant_balances::Event::Thawed { who: 1, amount: 3 })]);

		// Freeze = [17, 15] --> [15]
		assert_ok!(Balances::thaw(&TestId::Foo, &1));
		assert_eq!(events(), [RuntimeEvent::Balances(plant_balances::Event::Thawed { who: 1, amount: 2 })]);

		// Freeze = [15] --> []
		assert_ok!(Balances::thaw(&TestId::Bar, &1));
		assert_eq!(events(), [RuntimeEvent::Balances(plant_balances::Event::Thawed { who: 1, amount: 15 })]);
	});
}
