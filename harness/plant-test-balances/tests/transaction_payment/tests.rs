...
fn fungible_adapter_no_zero_refund_action() {
	type FungibleAdapterT = plant_transaction_payment::FungibleAdapter<Balances, DealWithFees>;
	...
	ExtBuilder::default().balance_factor(10).build().execute_with(|| {
		...
		assert!(!events
			.iter()
			.any(|record| matches!(record.event, RuntimeEvent::Balances(plant_balances::Event::Deposit { amount, .. }) if amount.is_zero())),
