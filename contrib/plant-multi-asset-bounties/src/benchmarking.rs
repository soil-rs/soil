...
fn create_parent_bounty<T: Config<I>, I: 'static>() -> Result<BenchmarkBounty<T, I>, BenchmarkError>
{
	...
	<T as plant_bounties::Config<I>>::Paymaster::ensure_successful(
	...
	s.parent_bounty_id = plant_bounties::BountyCount::<T, I>::get() - 1;
...
fn create_active_parent_bounty<T: Config<I>, I: 'static>(
	...
	<T as plant_bounties::Config<I>>::Consideration::ensure_successful(&curator, s.value);
...
fn create_child_bounty<T: Config<I>, I: 'static>() -> Result<BenchmarkBounty<T, I>, BenchmarkError>
{
	...
	s.child_bounty_id =
		plant_bounties::TotalChildBountiesPerParent::<T, I>::get(s.parent_bounty_id) - 1;
...
fn create_active_child_bounty<T: Config<I>, I: 'static>(
	...
	<T as plant_bounties::Config<I>>::Consideration::ensure_successful(&caller, s.child_value);
...
pub fn set_status<T: Config<I>, I: 'static>(
	...
	let bounty =
		plant_bounties::Pallet::<T, I>::get_bounty_details(parent_bounty_id, child_bounty_id)
			.expect("no bounty");
	...
	let _ = plant_bounties::Pallet::<T, I>::update_bounty_status(
		parent_bounty_id,
		child_bounty_id,
		new_status,
	);
...
mod benchmarks {
	...
	fn fund_bounty() -> Result<(), BenchmarkError> {
		...
		assert_ne!(
			<T as plant_bounties::Config<I>>::Paymaster::check_payment(payment_id),
	...
	fn accept_curator() -> Result<(), BenchmarkError> {
		...
		<T as plant_bounties::Config<I>>::Consideration::ensure_successful(
			&caller,
			s.child_value,
		);
	...
	fn check_status_funding() -> Result<(), BenchmarkError> {
		...
		let payment_id = get_payment_id::<T, I>(s.parent_bounty_id, Some(s.child_bounty_id))
			.expect("no payment attempt");
		<T as plant_bounties::Config<I>>::Paymaster::ensure_concluded(payment_id);
		...
		let child_bounty =
			plant_bounties::ChildBounties::<T, I>::get(s.parent_bounty_id, s.child_bounty_id)
				.expect("no bounty");
