...
pub mod pallet {
	...
	use topsoil_core::system::{ensure_root, pallet_prelude::*};
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		pub fn approve(
			...
			member.last_proof = topsoil_core::system::Pallet::<T>::block_number();
			Member::<T, I>::insert(&who, &member);
	...
	for EnsureInducted<T, I, MIN_RANK>
{
	...
	fn try_origin(o: T::RuntimeOrigin) -> Result<Self::Success, T::RuntimeOrigin> {
		let who = <topsoil_core::system::EnsureSigned<_> as EnsureOrigin<_>>::try_origin(o)?;
		match T::Members::rank_of(&who) {
			Some(rank) if rank >= MIN_RANK && Member::<T, I>::contains_key(&who) => Ok(who),
			_ => Err(topsoil_core::system::RawOrigin::Signed(who).into()),
		}
	}
	...
	for Pallet<T, I>
{
	fn ensure_member(who: &<T as topsoil_core::system::Config>::AccountId) {
		#[allow(deprecated)]
		Self::import(topsoil_core::system::RawOrigin::Signed(who.clone()).into()).unwrap();
	}
}
