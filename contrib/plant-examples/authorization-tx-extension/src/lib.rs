...
//! - [plant_assets] - a dummy asset pallet that tracks assets, identified by an
//!   [AssetId](plant_assets::AssetId), and their respective owners, which can be either an
//!   [account](plant_assets::Owner::Single) or a [pair of owners](plant_assets::Owner::Double).
//!
//! Assets are created in [plant_assets] using the
//! [create_asset](plant_assets::Call::create_asset) call, which accepts traditionally signed
//! origins (a single account) or coowner origins, authorized through the
//! [CoownerOrigin](plant_assets::Config::CoownerOrigin) type.
//!
