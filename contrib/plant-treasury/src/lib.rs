...
use topsoil_core::system::pallet_prelude::BlockNumberFor as SystemBlockNumberFor;
...
pub mod pallet {
	...
	use topsoil_core::system::pallet_prelude::{ensure_signed, OriginFor};
	...
	impl<T: Config<I>, I: 'static> Hooks<SystemBlockNumberFor<T>> for Pallet<T, I> {
		...
		fn try_state(_: SystemBlockNumberFor<T>) -> Result<(), subsoil::runtime::TryRuntimeError> {
			Self::do_try_state()?;
			Ok(())
		}
	}
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
	...
	#[cfg(any(feature = "try-runtime", test))]
	fn try_state_proposals() -> Result<(), subsoil::runtime::TryRuntimeError> {
	...
	/// > spend.valid_from.
	#[cfg(any(feature = "try-runtime", test))]
	fn try_state_spends() -> Result<(), subsoil::runtime::TryRuntimeError> {
...
impl<R> subsoil::runtime::traits::TypedGet for TreasuryAccountId<R>
where
	R: crate::Config,
{
	type Type = <R as topsoil_core::system::Config>::AccountId;
	fn get() -> Self::Type {
