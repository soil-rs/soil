...
impl plant_bags_list::Config<VoterBagsListInstance> for Test {
	type RuntimeEvent = RuntimeEvent;
	type WeightInfo = ();
...
impl ExtBuilder {
	...
	fn build(self) -> subsoil::io::TestExternalities {
		...
		let _ = plant_staking::GenesisConfig::<Test> {
...
pub(crate) fn active_era() -> EraIndex {
	plant_staking::ActiveEra::<Test>::get().unwrap().index
}

pub(crate) fn current_era() -> EraIndex {
	plant_staking::CurrentEra::<Test>::get().unwrap()
}
...
pub(crate) fn on_offence_in_era(
	...
	if plant_staking::ActiveEra::<Test>::get().unwrap().index == era {
		let _ = <Staking as OnOffenceHandler<_, _, _>>::on_offence(
			offenders,
			slash_fraction,
			plant_staking::ErasStartSessionIndex::<Test>::get(era).unwrap(),
		);
	} else {
...
pub(crate) fn on_offence_now(
	...
	let now = plant_staking::ActiveEra::<Test>::get().unwrap().index;
