...
type AllianceMotionEvent = plant_collective::Event<Test, plant_collective::Instance1>;
...
fn propose_works() {
	build_and_execute(|| {
		...
		assert_eq!(*plant_collective::Proposals::<Test, Instance1>::get(), vec![hash]);
		assert_eq!(plant_collective::ProposalOf::<Test, Instance1>::get(&hash), Some(proposal));
		assert_eq!(
			System::events(),
			vec![EventRecord {
