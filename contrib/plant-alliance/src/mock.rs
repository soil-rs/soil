...
type AllianceCollective = plant_collective::Instance1;
impl plant_collective::Config<AllianceCollective> for Test {
...
#[cfg(feature = "runtime-benchmarks")]
impl plant_identity::BenchmarkHelper<AccountU64, AccountU64> for BenchmarkHelper {
	fn sign_message(_message: &[u8]) -> (AccountU64, AccountU64) {
		(AccountU64(0), AccountU64(0))
	}
}

impl plant_identity::Config for Test {
	type RuntimeEvent = RuntimeEvent;
...
impl ProposalProvider<AccountId, H256, RuntimeCall> for AllianceProposalProvider {
	...
	fn proposal_of(proposal_hash: H256) -> Option<RuntimeCall> {
		plant_collective::ProposalOf::<Test, Instance1>::get(proposal_hash)
	}
}
...
pub fn new_test_ext() -> subsoil::io::TestExternalities {
	...
	plant_alliance::GenesisConfig::<Test> {
		fellows: vec![],
		allies: vec![],
...
pub fn make_kick_member_proposal(who: AccountId) -> (RuntimeCall, u32, H256) {
	make_proposal(RuntimeCall::Alliance(plant_alliance::Call::kick_member { who }))
}
