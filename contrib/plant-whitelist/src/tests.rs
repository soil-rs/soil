...
fn test_whitelist_call_and_remove() {
	new_test_ext().execute_with(|| {
		...
		let call_hash = <Test as topsoil_core::system::Config>::Hashing::hash(&encoded_call[..]);
...
fn test_whitelist_call_and_execute() {
	new_test_ext().execute_with(|| {
		let call = RuntimeCall::System(topsoil_core::system::Call::remark_with_event { remark: vec![1] });
...
fn test_whitelist_call_and_execute_without_note_preimage() {
	new_test_ext().execute_with(|| {
		let call = Box::new(RuntimeCall::System(topsoil_core::system::Call::remark_with_event {
			remark: vec![1],
		}));
		let call_hash = <Test as topsoil_core::system::Config>::Hashing::hash_of(&call);
...
fn test_whitelist_call_and_execute_decode_consumes_all() {
	new_test_ext().execute_with(|| {
		...
		let call_hash = <Test as topsoil_core::system::Config>::Hashing::hash(&call[..]);
