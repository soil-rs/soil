	...
	impl<T: Config> Pallet<T> {
		...
		pub fn add_child_bounty(
			...
			let parent_bounty_account =
				plant_bounties::Pallet::<T>::bounty_account_id(parent_bounty_id);
...
impl<T: Config> Pallet<T> {
	...
	pub fn treasury_block_number() -> BlockNumberFor<T> {
		<T as plant_treasury::Config>::BlockNumberProvider::current_block_number()
	}
	...
	fn calculate_curator_deposit(
		...
		plant_bounties::Pallet::<T>::calculate_curator_deposit(bounty_fee)
	}
	...
	fn ensure_bounty_active(
		...
		let parent_bounty = plant_bounties::Bounties::<T>::get(bounty_id)
...
/// fees during call.
impl<T: Config> plant_bounties::ChildBountyManager<BalanceOf<T>> for Pallet<T> {
	/// Returns number of active child bounties for `bounty_id`
	fn child_bounties_count(
		bounty_id: plant_bounties::BountyIndex,
	) -> plant_bounties::BountyIndex {
	...
	fn children_curator_fees(bounty_id: plant_bounties::BountyIndex) -> BalanceOf<T> {
