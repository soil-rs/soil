...
fn go_to_block(n: u64) {
	<Test as plant_treasury::Config>::BlockNumberProvider::set_block_number(n);
...
type TreasuryError = plant_treasury::Error<Test>;
type TreasuryError1 = plant_treasury::Error<Test, Instance1>;
...
impl ExtBuilder {
	pub fn build(self) -> subsoil::io::TestExternalities {
		...
		.into();
		ext.execute_with(|| {
			<Test as plant_treasury::Config>::BlockNumberProvider::set_block_number(1)
		});
		ext
	}
...
fn propose_bounty_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			plant_bounties::Bounties::<Test>::get(0).unwrap(),
			Bounty {
				proposer: 0,
				fee: 0,
				curator_deposit: 0,
		...
		assert_eq!(
			plant_bounties::BountyDescriptions::<Test>::get(0).unwrap(),
			b"1234567890".to_vec()
		);

		assert_eq!(plant_bounties::BountyCount::<Test>::get(), 1);
	});
}
...
fn close_bounty_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(plant_bounties::Bounties::<Test>::get(0), None);
		assert!(!plant_treasury::Proposals::<Test>::contains_key(0));

		assert_eq!(plant_bounties::BountyDescriptions::<Test>::get(0), None);
	});
}
...
fn close_bounty_with_additional_assets_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert!(matches!(
			plant_bounties::Bounties::<Test>::get(0).unwrap().status,
			BountyStatus::Active { .. }
		));
...
fn approve_bounty_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(plant_bounties::BountyApprovals::<Test>::get(), vec![0]);
...
fn test_migration_v4() {
	...
	let data = vec![
		(plant_bounties::BountyCount::<Test>::hashed_key().to_vec(), 10.encode().to_vec()),
		(plant_bounties::Bounties::<Test>::hashed_key_for(index), bounty.encode().to_vec()),
		(plant_bounties::BountyDescriptions::<Test>::hashed_key_for(index), vec![0, 0]),
		(
			plant_bounties::BountyApprovals::<Test>::hashed_key().to_vec(),
...
fn genesis_funding_works() {
	...
	.assimilate_storage(&mut t)
	.unwrap();
	plant_treasury::GenesisConfig::<Test>::default()
		.assimilate_storage(&mut t)
		.unwrap();
	let mut t: subsoil::io::TestExternalities = t.into();
...
fn approve_bounty_with_curator_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(plant_bounties::BountyApprovals::<Test>::get().len(), 0);
		assert_ok!(Bounties::approve_bounty_with_curator(RuntimeOrigin::root(), 0, curator, 10));
		assert_eq!(plant_bounties::BountyApprovals::<Test>::get().len(), 1);
...
fn accept_curator_sets_update_due_correctly() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// Then
		assert_eq!(
			plant_bounties::Bounties::<Test>::get(bounty_id).unwrap().status,
		...
		// Then
		assert_eq!(
			plant_bounties::Bounties::<Test, Instance1>::get(bounty_id).unwrap().status,
...
fn poke_deposit_fails_for_insufficient_balance() {
	ExtBuilder::default().build_and_execute(|| {
		...
		let deposit =
			plant_bounties::Pallet::<Test>::calculate_bounty_deposit(&bounded_description);
		...
		assert_eq!(plant_bounties::BountyDescriptions::<Test>::get(0).unwrap(), description);
		assert_eq!(plant_bounties::BountyCount::<Test>::get(), 1);
...
fn poke_deposit_fails_for_any_status_other_than_proposed() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(plant_bounties::Bounties::<Test>::get(0).unwrap(), bounty);
		assert_eq!(plant_bounties::BountyDescriptions::<Test>::get(0).unwrap(), description);
		assert_eq!(plant_bounties::BountyCount::<Test>::get(), 1);

		// Change status to approved
		bounty.status = BountyStatus::Approved;
		plant_bounties::Bounties::<Test>::insert(bounty_id, &bounty);
