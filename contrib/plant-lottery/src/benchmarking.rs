...
fn setup_lottery<T: Config>(repeat: bool) -> Result<(), &'static str> {
	...
	let mut calls = vec![
		topsoil_core::system::Call::<T>::set_code { code: vec![] }.into();
		T::MaxCalls::get().saturating_sub(1) as usize
	];
	// Last call will be the match for worst case scenario.
	calls.push(topsoil_core::system::Call::<T>::remark { remark: vec![] }.into());
...
mod benchmarks {
	...
	fn set_calls(n: Linear<0, { T::MaxCalls::get() }>) -> Result<(), BenchmarkError> {
		let calls = vec![topsoil_core::system::Call::<T>::remark { remark: vec![] }.into(); n as usize];
		let origin =
