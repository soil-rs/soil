...
use soil_test_staging_node_primitives::{BlockNumber, Hash};
...
pub fn compact_code_unwrap() -> &'static [u8] {
	soil_test_staging_node_runtime::WASM_BINARY.expect(
...
pub const SPEC_VERSION: u32 = soil_test_staging_node_runtime::VERSION.spec_version;

pub const TRANSACTION_VERSION: u32 = soil_test_staging_node_runtime::VERSION.transaction_version;
...
pub fn new_test_ext(code: &[u8]) -> TestExternalities<BlakeTwo256> {
	...
	let ext = TestExternalities::new_with_code(
		code,
		soil_test_staging_node_testing::genesis::config().build_storage().unwrap(),
	);
	ext
}
