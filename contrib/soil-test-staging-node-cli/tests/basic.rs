...
use soil_test_staging_node_primitives::{Balance, Hash};
...
pub fn bloaty_code_unwrap() -> &'static [u8] {
	soil_test_staging_node_runtime::WASM_BINARY_BLOATY.expect(
...
fn xt() -> UncheckedExtrinsic {
	sign(CheckedExtrinsic {
		format: subsoil::runtime::generic::ExtrinsicFormat::Signed(alice(), tx_ext(0, 0)),
	...
	construct_block(
		...
		vec![
			CheckedExtrinsic {
				...
				function: RuntimeCall::Timestamp(plant_timestamp::Call::set { now: time }),
			},
			CheckedExtrinsic {
				format: subsoil::runtime::generic::ExtrinsicFormat::Signed(alice(), tx_ext(0, 0)),
...
fn blocks() -> ((Vec<u8>, Hash), (Vec<u8>, Hash)) {
	...
	let block2 = construct_block(
		...
		vec![
			...
			CheckedExtrinsic {
				format: subsoil::runtime::generic::ExtrinsicFormat::Signed(bob(), tx_ext(0, 0)),
				function: RuntimeCall::Balances(plant_balances::Call::transfer_allow_death {
					dest: alice().into(),
					value: 5 * DOLLARS,
				}),
			...
			CheckedExtrinsic {
				format: subsoil::runtime::generic::ExtrinsicFormat::Signed(alice(), tx_ext(1, 0)),
				function: RuntimeCall::Balances(plant_balances::Call::transfer_allow_death {
					dest: bob().into(),
					value: 15 * DOLLARS,
				}),
...
fn block_with_size(time: u64, nonce: u32, size: usize) -> (Vec<u8>, Hash) {
	construct_block(
		...
		vec![
			CheckedExtrinsic {
				format: subsoil::runtime::generic::ExtrinsicFormat::Bare,
				function: RuntimeCall::Timestamp(plant_timestamp::Call::set { now: time * 1000 }),
			},
			CheckedExtrinsic {
...
fn full_native_block_import_works() {
	...
	let timestamp_weight = plant_timestamp::Call::set::<Runtime> { now: Default::default() }
		.get_dispatch_info()
		.call_weight
		.saturating_add(
			<Runtime as topsoil_system::Config>::BlockWeights::get()
	...
	t.execute_with(|| {
		...
		let events = vec![
			...
			EventRecord {
				...
				event: RuntimeEvent::Balances(plant_balances::Event::Withdraw {
					who: alice().into(),
					amount: fees,
				}),
				topics: vec![],
			...
			EventRecord {
				phase: Phase::ApplyExtrinsic(1),
				event: RuntimeEvent::Balances(plant_balances::Event::Transfer {
					from: alice().into(),
					to: bob().into(),
					amount: 69 * DOLLARS,
				}),
			...
			EventRecord {
				phase: Phase::ApplyExtrinsic(1),
				event: RuntimeEvent::Balances(plant_balances::Event::Deposit {
			...
			EventRecord {
				...
				event: RuntimeEvent::TransactionPayment(
					plant_transaction_payment::Event::TransactionFeePaid {
						who: alice().into(),
...
fn should_import_block_with_test_client() {
	use soil_test_staging_node_testing::client::{
		BlockOrigin, ClientBlockImportExt, TestClientBuilder, TestClientBuilderExt,
	};

	let client = TestClientBuilder::new().build();
	let block1 = changes_trie_block();
	let block_data = block1.0;
	let block = soil_test_staging_node_primitives::Block::decode(&mut &block_data[..]).unwrap();
