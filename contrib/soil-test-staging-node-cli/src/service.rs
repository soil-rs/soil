...
use codec::Encode;
use futures::prelude::*;
use soil_test_staging_node_runtime::RuntimeApi;
use soil_test_staging_node_primitives::Block;
...
pub fn create_extrinsic(
	...
	function: impl Into<soil_test_staging_node_runtime::RuntimeCall>,
	nonce: Option<u32>,
) -> soil_test_staging_node_runtime::UncheckedExtrinsic {
	...
	let period = soil_test_staging_node_runtime::BlockHashCount::get()
	...
	let raw_payload = soil_test_staging_node_runtime::SignedPayload::from_raw(
		...
		(
			(),
			(),
			soil_test_staging_node_runtime::VERSION.spec_version,
			soil_test_staging_node_runtime::VERSION.transaction_version,
			genesis_hash,
			best_hash,
			(),
			(),
	...
	generic::UncheckedExtrinsic::new_signed(
		...
		soil_test_staging_node_runtime::Signature::Sr25519(signature),
		tx_ext,
	)
	.into()
}
...
> {
	...
	let babe_config = soil_babe::configuration(&*client)?;
	let slot_duration = babe_config.slot_duration();
	let (block_import, babe_link) = soil_babe::block_import(
		babe_config,
		beefy_block_import,
		client.clone(),
	...
	let (import_queue, babe_worker_handle) =
		soil_babe::import_queue(soil_babe::ImportQueueParams {
	...
	let (rpc_extensions_builder, rpc_setup) = {
		...
		let rpc_extensions_builder =
			move |subscription_executor: soil_test_staging_node_rpc::SubscriptionTaskExecutor| {
				let deps = soil_test_staging_node_rpc::FullDeps {
					client: client.clone(),
					pool: pool.clone(),
					select_chain: select_chain.clone(),
					chain_spec: chain_spec.cloned_box(),
					babe: soil_test_staging_node_rpc::BabeDeps {
						keystore: keystore.clone(),
						babe_worker_handle: babe_worker_handle.clone(),
					},
					grandpa: soil_test_staging_node_rpc::GrandpaDeps {
					...
					beefy: soil_test_staging_node_rpc::BeefyDeps::<beefy_primitives::ecdsa_crypto::AuthorityId> {
					...
					statement_store_deps: soil_test_staging_node_rpc::StatementStoreDeps {
				...
				soil_test_staging_node_rpc::create_full(deps).map_err(Into::into)
			};
...
pub fn new_full_base<N: NetworkBackend<Block, <Block as BlockT>::Hash>>(
	...
	let backoff_authoring_blocks =
		Some(soil_consensus::slots::BackoffAuthoringOnFinalizedHeadLagging::default());
	...
	if let soil_service::config::Role::Authority { .. } = &role {
		...
		let babe_config = soil_babe::BabeParams {
			keystore: keystore_container.keystore(),
			client: client.clone(),
			select_chain,
			env: proposer,
		...
		let babe = soil_babe::start_babe(babe_config)?;
	...
	if enable_offchain_worker {
		let offchain_workers =
			soil_offchain::OffchainWorkers::new(soil_offchain::OffchainWorkerOptions {
...
mod tests {
	...
	use soil_test_staging_node_primitives::{Block, DigestItem, Signature};
	...
	#[test]
	// It is "ignored", but the soil-test-staging-node-cli ignored tests are running on the CI.
	// This can be run locally with `cargo test --release -p soil-test-staging-node-cli test_sync -- --ignored`.
	#[ignore]
	fn test_sync() {
		...
		soil_test::service::sync(
			...
			|config| {
				...
				let NewFullBase { task_manager, client, network, sync, transaction_pool, .. } =
					new_full_base::<soil_network::NetworkWorker<_, _>>(
						...
						1,
						50_000,
						|block_import: &soil_babe::BabeBlockImport<Block, _, _, _, _>,
						 babe_link: &soil_babe::BabeLink<Block>| {
				...
				let (babe_pre_digest, epoch_descriptor) = loop {
					...
					let epoch = babe_link
						...
						.epoch_data(&epoch_descriptor, |slot| {
							soil_babe::Epoch::genesis(babe_link.config(), slot)
						})
						.unwrap();

					if let Some(babe_pre_digest) =
						soil_babe::authorship::claim_slot(slot.into(), &epoch, &keystore)
							.map(|(digest, _)| digest)
					{
			...
			|service, _| {
				...
				let (spec_version, transaction_version) = {
					let version = service
					.client()
					.runtime_version_at(best_hash, subsoil::api::CallContext::Offchain)
					.unwrap();
				...
				index += 1;
				let utx: soil_test_staging_node_runtime::UncheckedExtrinsic =
					generic::UncheckedExtrinsic::new_signed(
						function,
						from.into(),
						signature.into(),
