...
/// migration.
pub fn migrate<T: plant_tips::Config, P: GetStorageVersion + PalletInfoAccess, N: AsRef<str>>(
	old_pallet_name: N,
) -> Weight {
	...
	if on_chain_storage_version < 4 {
		let storage_prefix = plant_tips::Tips::<T>::storage_prefix();
		...
		let storage_prefix = plant_tips::Reasons::<T>::storage_prefix();
		topsoil_core::storage::migration::move_storage_from_pallet(
			storage_prefix,
			old_pallet_name.as_bytes(),
			new_pallet_name.as_bytes(),
...
>(
	...
	let storage_prefix_tips = plant_tips::Tips::<T>::storage_prefix();
	let storage_prefix_reasons = plant_tips::Reasons::<T>::storage_prefix();
...
>(
	...
	let old_tips_key_iter = topsoil_core::storage::KeyPrefixIterator::new(
		old_tips_key.to_vec(),
		old_tips_key.to_vec(),
		|_| Ok(()),
	);
	...
	let old_reasons_key_iter = topsoil_core::storage::KeyPrefixIterator::new(
