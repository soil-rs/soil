...
fn test_last_reward_migration() {
	...
	let data = vec![
		(plant_tips::Tips::<Test>::hashed_key_for(hash1), old_tip_finder.encode().to_vec()),
		(plant_tips::Tips::<Test>::hashed_key_for(hash2), old_tip_no_finder.encode().to_vec()),
	];

	s.top = data.into_iter().collect();

	subsoil::io::TestExternalities::new(s).execute_with(|| {
		let module = plant_tips::Tips::<Test>::pallet_prefix();
		let item = plant_tips::Tips::<Test>::storage_prefix();
		Tips::migrate_retract_tip_for_tip_new(module, item);

		// Test w/ finder
		assert_eq!(
			plant_tips::Tips::<Test>::get(hash1),
			Some(OpenTip {
				reason: reason1,
				who: 10,
				finder: 20,
		...
		// Test w/o finder
		assert_eq!(
			plant_tips::Tips::<Test>::get(hash2),
			Some(OpenTip {
				reason: reason2,
				who: 20,
...
fn test_migration_v4() {
	...
	let data = vec![
		(plant_tips::Reasons::<Test>::hashed_key_for(hash1), reason1.encode().to_vec()),
		(plant_tips::Tips::<Test>::hashed_key_for(hash1), tip.encode().to_vec()),
	];
...
fn equal_entries_invariant() {
	new_test_ext().execute_with(|| {
		...
		plant_tips::Tips::<Test>::insert(hash1, tip);
...
fn reasons_invariant() {
	new_test_ext().execute_with(|| {
		...
		let hash: Vec<_> = plant_tips::Tips::<Test>::iter_keys().collect();
