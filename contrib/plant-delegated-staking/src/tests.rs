...
mod staking_integration {
	...
	fn migration_works() {
		ExtBuilder::default().build_and_execute(|| {
			...
			assert_eq!(plant_staking::asset::total_balance::<T>(&agent), agent_amount);
...
mod pool_integration {
	use super::*;
	use plant_nomination_pools::{BondExtra, BondedPools, PoolState};
	...
	fn create_pool(creator: AccountId, amount: Balance) -> u32 {
		...
		plant_nomination_pools::LastPoolId::<T>::get()
	}
