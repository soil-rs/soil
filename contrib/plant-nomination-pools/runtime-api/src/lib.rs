...
subsoil::api::decl_runtime_apis! {
			...
			Balance: Codec,
	{
		...
		/// [`DelegateStake`](plant_nomination_pools::adapter::DelegateStake) but the pool
		/// still has funds that were staked using the older strategy
		/// [TransferStake](plant_nomination_pools::adapter::TransferStake). Use
		/// [`migrate_pool_to_delegate_stake`](plant_nomination_pools::Call::migrate_pool_to_delegate_stake)
		/// to migrate the pool.
		...
		/// [`DelegateStake`](plant_nomination_pools::adapter::DelegateStake), the funds of the
		/// member can be migrated from pool account to the member's account. Use
		/// [`migrate_delegation`](plant_nomination_pools::Call::migrate_delegation)
