...
use topsoil_core::traits::{OnRuntimeUpgrade, UncheckedOnRuntimeUpgrade};
...
pub mod versioned {
	...
	pub type V7ToV8<T> = topsoil_core::migrations::VersionedMigration<
		7,
		8,
	...
	/// Migration V6 to V7 wrapped in a [`topsoil_core::migrations::VersionedMigration`], ensuring
	/// the migration is only performed when on-chain version is 6.
	pub type V6ToV7<T> = topsoil_core::migrations::VersionedMigration<
		6,
		7,
	...
	pub type V5toV6<T> = topsoil_core::migrations::VersionedMigration<
		5,
		6,
		v6::MigrateToV6<T>,
		crate::pallet::Pallet<T>,
	...
	fn migration_assumption_is_correct() {
		...
		ExtBuilder::default().build_and_execute(|| {
			let join = |x| {
				...
				topsoil_core::assert_ok!(Pools::join(RuntimeOrigin::signed(x), 10, 1));
			};
	...
	impl<T: Config> OnRuntimeUpgrade for MigrateToV2<T> {
		...
		fn pre_upgrade() -> Result<Vec<u8>, TryRuntimeError> {
			...
			RewardPools::<T>::iter().try_for_each(|(id, _)| -> Result<(), TryRuntimeError> {
					...
					<T::Currency as topsoil_core::traits::fungible::Inspect<T::AccountId>>::balance(&Pallet::<T>::generate_reward_account(id)) >=
...
pub mod v1 {
	...
	impl<T: Config> OnRuntimeUpgrade for MigrateToV1<T> {
		...
		fn post_upgrade(_: Vec<u8>) -> Result<(), TryRuntimeError> {
			...
			Pallet::<T>::try_state(topsoil_core::system::Pallet::<T>::block_number())?;
			Ok(())
		}
	}
}
