...
use subsoil::staking::{EraIndex, StakingUnchecked};
use subsoil::runtime::{
...
type CurrencyOf<T> = <T as plant_nomination_pools::Config>::Currency;

const USER_SEED: u32 = 0;
const MAX_SPANS: u32 = 100;

pub(crate) type VoterBagsListInstance = plant_bags_list::Instance1;
pub trait Config:
	plant_nomination_pools::Config
	+ plant_staking::Config
	+ plant_bags_list::Config<VoterBagsListInstance>
{
}

pub struct Pallet<T: Config>(Pools<T>);

fn create_funded_user_with_balance<T: plant_nomination_pools::Config>(
	string: &'static str,
	n: u32,
	balance: BalanceOf<T>,
) -> T::AccountId {
...
// balance.
fn create_pool_account<T: plant_nomination_pools::Config>(
	n: u32,
	balance: BalanceOf<T>,
	...
	if let Some(c) = commission {
		let pool_id = plant_nomination_pools::LastPoolId::<T>::get();
	...
	let pool_account = plant_nomination_pools::BondedPools::<T>::iter()
...
fn migrate_to_transfer_stake<T: Config>(pool_id: PoolId) {
	...
	PoolMembers::<T>::iter()
		...
		.for_each(|(member_acc, member)| {
			...
			<T as plant_nomination_pools::Config>::Currency::transfer(
				&member_acc,
				&pool_acc,
				member_balance,
	...
	plant_staking::Pallet::<T>::migrate_to_direct_staker(&pool_acc);
}

fn vote_to_balance<T: plant_nomination_pools::Config>(
	vote: u64,
...
#[allow(unused)]
struct ListScenario<T: plant_nomination_pools::Config> {
...
impl<T: Config> ListScenario<T> {
	...
	pub(crate) fn new(
		...
		is_increase: bool,
			...
			plant_nomination_pools::MaxPools::<T>::get().unwrap_or(0) >= 3,
		...
		let dest_weight_as_vote =
			<T as plant_staking::Config>::VoterList::score_update_worst_case(
				&pool_origin1,
				is_increase,
			);
		...
		let weight_of = plant_staking::Pallet::<T>::weight_of_fn();
...
#[benchmarks(
	where
		T: plant_staking::Config,
		plant_staking::BalanceOf<T>: From<u128>,
...
mod benchmarks {
	...
	fn pool_withdraw_unbonded(s: Linear<0, MAX_SPANS>) {
		...
		assert_eq!(plant_staking::Ledger::<T>::get(&pool_account).unwrap().unlocking.len(), 1);
		// Set the current era
		plant_staking::CurrentEra::<T>::put(EraIndex::max_value());

		// Add `s` count of slashing spans to storage.
		plant_staking::benchmarking::add_slashing_spans::<T>(&pool_account, s);
		...
		assert_eq!(plant_staking::Ledger::<T>::get(pool_account).unwrap().unlocking.len(), 0);
	}
	...
	fn withdraw_unbonded_update(s: Linear<0, MAX_SPANS>) {
		...
		// Unbond the new member
		plant_staking::CurrentEra::<T>::put(0);
		Pools::<T>::fully_unbond(RuntimeOrigin::Signed(joiner.clone()).into(), joiner.clone())
			.unwrap();
	...
	fn withdraw_unbonded_kill(s: Linear<0, MAX_SPANS>) {
		...
		assert!(plant_staking::Ledger::<T>::contains_key(&pool_account));
	...
	#[benchmark]
	fn set_metadata(
		n: Linear<1, { <T as plant_nomination_pools::Config>::MaxMetadataLen::get() }>,
	) {
		// Create a pool
		let (depositor, _pool_account) =
	...
	#[benchmark]
	fn update_roles() {
		let first_id = plant_nomination_pools::LastPoolId::<T>::get() + 1;
		let (root, _) =
			create_pool_account::<T>(0, Pools::<T>::depositor_min_bond() * 2u32.into(), None);
		...
		assert_eq!(
			plant_nomination_pools::BondedPools::<T>::get(first_id).unwrap().roles,
			plant_nomination_pools::PoolRoles {
				depositor: root,
	...
	fn apply_slash() {
		...
		// slash pool by half
		plant_staking::slashing::do_slash::<T>(
			&pool_account,
			slash_amount.into(),
			&mut plant_staking::BalanceOf::<T>::zero(),
			&mut plant_staking::NegativeImbalanceOf::<T>::zero(),
			EraIndex::zero(),
		);
		...
		for i in 1..(T::MaxUnbonding::get() + 1) {
			plant_staking::CurrentEra::<T>::put(i);
		...
		plant_staking::CurrentEra::<T>::put(T::MaxUnbonding::get() + 2);
	...
	fn apply_slash_fail() {
		...
		plant_staking::CurrentEra::<T>::put(1);
		...
		for i in 0..T::MaxUnbonding::get() {
			plant_staking::CurrentEra::<T>::put(i + 2); // +2 because we already set the current era to 1.
			assert!(Pools::<T>::unbond(
		...
		plant_staking::CurrentEra::<T>::put(T::MaxUnbonding::get() + 3);
		whitelist_account!(joiner);
