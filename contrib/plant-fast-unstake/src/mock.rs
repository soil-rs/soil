...
use plant_staking::{Exposure, IndividualExposure, StakerStatus};
	...
	pub(crate) fn register_stakers_for_era(era: u32) {
		...
		(VALIDATOR_PREFIX..VALIDATOR_PREFIX + VALIDATORS_PER_ERA)
			...
			.for_each(|(validator, exposure)| {
				plant_staking::EraInfo::<T>::set_exposure(era, &validator, exposure);
			});
	}
	...
	pub(crate) fn build(self) -> subsoil::io::TestExternalities {
		...
		let _ = plant_staking::GenesisConfig::<Runtime> {
			stakers: self
				.unexposed
				.into_iter()
				.map(|(x, y, z)| (x, y, z, plant_staking::StakerStatus::Nominator(vec![42])))
		...
		ext.execute_with(|| {
			...
			plant_staking::ValidatorCount::<Runtime>::put(VALIDATORS_PER_ERA as u32);
		});
...
pub fn assert_unstaked(stash: &AccountId) {
	assert!(!plant_staking::Bonded::<T>::contains_key(stash));
	assert!(!plant_staking::Payee::<T>::contains_key(stash));
	assert!(!plant_staking::Validators::<T>::contains_key(stash));
	assert!(!plant_staking::Nominators::<T>::contains_key(stash));
}

pub fn create_exposed_nominator(exposed: AccountId, era: u32) {
	// create an exposed nominator in passed era
	let mut exposure = plant_staking::EraInfo::<T>::get_full_exposure(era, &VALIDATORS_PER_ERA);
	exposure.others.push(IndividualExposure { who: exposed, value: 0 as Balance });
	plant_staking::EraInfo::<T>::set_exposure(era, &VALIDATORS_PER_ERA, exposure);
