...
use soil_test_staging_node_testing::bench::{BenchDb, BlockType, DatabaseType, KeyTypes};
...
impl core::Benchmark for ConstructionBenchmark {
	fn run(&mut self, mode: Mode) -> std::time::Duration {
		...
		let _ = context
			.client
			.runtime_version_at(context.client.chain_info().genesis_hash, subsoil::api::CallContext::Offchain)
...
impl soil_client::transaction_pool::TransactionPool for Transactions {
	...
	) -> Result<Vec<Result<soil_test_staging_node_primitives::Hash, Self::Error>>, Self::Error> {
		unimplemented!()
	}
