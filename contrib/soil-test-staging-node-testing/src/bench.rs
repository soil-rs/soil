...
pub struct BlockContentIterator<'a> {
	...
	genesis_hash: soil_test_staging_node_primitives::Hash,
		...
		let signed = self.keyring.sign(
			CheckedExtrinsic {
				format: ExtrinsicFormat::Signed(
					sender,
					tx_ext(0, soil_test_staging_node_runtime::ExistentialDeposit::get() + 1),
				),
				function: match self.content.block_type {
					BlockType::RandomTransfersKeepAlive => {
						RuntimeCall::Balances(BalancesCall::transfer_keep_alive {
							dest: subsoil::runtime::MultiAddress::Id(receiver),
							value: soil_test_staging_node_runtime::ExistentialDeposit::get() + 1,
						})
					},
					BlockType::RandomTransfersReaping => {
						RuntimeCall::Balances(BalancesCall::transfer_allow_death {
							...
							value: 100 * DOLLARS
								- (soil_test_staging_node_runtime::ExistentialDeposit::get() - 1),
						})
					},
					BlockType::Noop => {
...
impl BenchKeyring {
	...
	/// soil_test_staging_node_runtime code blob.
