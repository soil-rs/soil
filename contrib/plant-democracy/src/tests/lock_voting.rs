...
fn the_lock(amount: u64) -> BalanceLock<u64> {
	BalanceLock { id: DEMOCRACY_ID, amount, reasons: plant_balances::Reasons::All }
}
...
fn lock_voting_should_work() {
	new_test_ext().execute_with(|| {
		...
		for i in 1..=5 {
			assert_eq!(plant_balances::Locks::<Test>::get(&i), vec![the_lock(i * 10)]);
		}
		...
		assert_eq!(plant_balances::Locks::<Test>::get(&1), vec![]);
		assert_eq!(plant_balances::Locks::<Test>::get(&2), vec![the_lock(20)]);
		assert_eq!(plant_balances::Locks::<Test>::get(&3), vec![the_lock(30)]);
		assert_eq!(plant_balances::Locks::<Test>::get(&4), vec![the_lock(40)]);
		assert_eq!(plant_balances::Locks::<Test>::get(&5), vec![]);
		...
		assert_ok!(Democracy::unlock(RuntimeOrigin::signed(1), 4));
		assert_eq!(plant_balances::Locks::<Test>::get(&4), vec![]);
		...
		assert_ok!(Democracy::unlock(RuntimeOrigin::signed(1), 3));
		assert_eq!(plant_balances::Locks::<Test>::get(&3), vec![]);
		...
		fast_forward_to(26);
		assert_ok!(Democracy::unlock(RuntimeOrigin::signed(1), 2));
		assert_eq!(plant_balances::Locks::<Test>::get(&2), vec![]);
	});
}
...
fn prior_lockvotes_should_be_enforced() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(plant_balances::Locks::<Test>::get(&5), vec![the_lock(50)]);
		fast_forward_to(8);
		assert_ok!(Democracy::remove_other_vote(RuntimeOrigin::signed(1), 5, r.2));
		assert_ok!(Democracy::unlock(RuntimeOrigin::signed(5), 5));
		assert_eq!(plant_balances::Locks::<Test>::get(&5), vec![the_lock(20)]);
		fast_forward_to(13);
		assert_noop!(
		...
		assert_ok!(Democracy::unlock(RuntimeOrigin::signed(5), 5));
		assert_eq!(plant_balances::Locks::<Test>::get(&5), vec![the_lock(10)]);
		fast_forward_to(25);
		assert_noop!(
...
fn multi_consolidation_of_lockvotes_should_be_conservative() {
	new_test_ext().execute_with(|| {
		...
		fast_forward_to(8);
		assert_ok!(Democracy::unlock(RuntimeOrigin::signed(5), 5));
		assert!(plant_balances::Locks::<Test>::get(&5)[0].amount >= 20);

		fast_forward_to(14);
		assert_ok!(Democracy::unlock(RuntimeOrigin::signed(5), 5));
		assert!(plant_balances::Locks::<Test>::get(&5)[0].amount >= 10);
...
fn locks_should_persist_from_delegation_to_voting() {
	new_test_ext().execute_with(|| {
		...
		fast_forward_to(26);
		assert_ok!(Democracy::unlock(RuntimeOrigin::signed(5), 5));
		assert!(plant_balances::Locks::<Test>::get(&5)[0].amount >= 5);
