...
/// [`storage::append`](topsoil::deps::subsoil::io::storage::append). It allows to directly extend
...
/// created through [`storage::append`](topsoil::deps::subsoil::io::storage::append). **Draining** advances
	...
	ValuesPerNewPage: Get<u32>,
{
	pub fn from_storage() -> Option<Self> {
		...
		subsoil::io::storage::get(&key).and_then(|raw| Self::decode(&mut &raw[..]).ok())
	}
	...
	pub fn store(&self) {
		let key = Self::key();
		self.using_encoded(|enc| subsoil::io::storage::set(&key, enc));
	}
	...
	pub fn delete() {
		subsoil::io::storage::clear(&Self::key());
	}
}
...
impl<V: FullCodec> Page<V> {
	...
	pub fn from_storage<Prefix: StorageInstance>(
		...
		let values = subsoil::io::storage::get(&key)
...
mod tests {
	...
	fn drain_eager_page_removal() {
		TestExternalities::default().execute_with(|| {
			...
			assert!(subsoil::io::storage::exists(&page_key::<Prefix>(0)));
			assert!(subsoil::io::storage::exists(&page_key::<Prefix>(1)));
	...
	fn append_storage_layout() {
		TestExternalities::default().execute_with(|| {
			...
			let raw = subsoil::io::storage::get(&key).expect("Page should be present");
			...
			let meta =
				subsoil::io::storage::get(&meta_key::<Prefix>()).expect("Meta should be present");
