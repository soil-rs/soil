...
use std::sync::Arc;
use plant_election_provider_multi_phase::{
...
parameter_types! {
	...
	pub const SessionsPerEra: subsoil::staking::SessionIndex = 2;
	pub static BondingDuration: subsoil::staking::EraIndex = 28;
	pub const SlashDeferDuration: subsoil::staking::EraIndex = 7; // 1/4 the bonding duration.
}

impl plant_bags_list::Config for Runtime {
	type RuntimeEvent = RuntimeEvent;
	type WeightInfo = ();
...
impl onchain::Config for OnChainSeqPhragmen {
	...
	type Solver = SequentialPhragmen<
		AccountId,
		plant_election_provider_multi_phase::SolutionAccuracyOf<Runtime>,
	>;
	type DataProvider = Staking;
	type WeightInfo = ();
...
impl plant_election_provider_multi_phase::BenchmarkingConfig
...
impl ExtBuilder {
	pub fn build(&self) -> subsoil::io::TestExternalities {
		...
		let _ = plant_staking::GenesisConfig::<Runtime> {
...
pub fn roll_to_epm_signed() {
	while !matches!(
		...
		plant_election_provider_multi_phase::Phase::Signed
	) {
...
pub fn roll_to_epm_unsigned() {
	while !matches!(
		CurrentPhase::<Runtime>::get(),
		plant_election_provider_multi_phase::Phase::Unsigned(_)
	) {
		roll_to(System::block_number() + 1, false);
	}
}
...
pub fn roll_to_epm_off() {
	while !matches!(
		CurrentPhase::<Runtime>::get(),
		plant_election_provider_multi_phase::Phase::Off
	) {
		roll_to(System::block_number() + 1, false);
	}
}
...
pub(crate) fn staking_events() -> Vec<plant_staking::Event<Runtime>> {
	System::events()
		.into_iter()
		.map(|r| r.event)
...
pub(crate) fn epm_events() -> Vec<plant_election_provider_multi_phase::Event<Runtime>> {
	System::events()
		.into_iter()
		.map(|r| r.event)
		.filter_map(|e| {
