...
use plant_election_provider_multi_phase::CurrentPhase;
...
fn log_current_time() {
	log!(
		...
		plant_staking::CurrentEra::<Runtime>::get(),
...
fn offchainify_works() {
	use plant_election_provider_multi_phase::QueuedSolution;
...
fn mass_slash_doesnt_enter_emergency_phase() {
	...
	ext.execute_with(|| {
		assert_eq!(
			plant_staking::ForceEra::<Runtime>::get(),
			plant_staking::Forcing::NotForcing
		);
...
fn ledger_consistency_active_balance_below_ed() {
	...
	execute_with(ext, || {
		...
		advance_eras(
			<Runtime as plant_staking::Config>::BondingDuration::get() as usize,
			pool_state,
		);
		assert_ok!(Staking::withdraw_unbonded(RuntimeOrigin::signed(11), 0));
...
fn automatic_unbonding_pools() {
	use plant_nomination_pools::TotalValueLocked;
	...
	execute_with(ext, || {
		assert_eq!(<Runtime as plant_staking::Config>::MaxUnlockingChunks::get(), 1);
		assert_eq!(<Runtime as plant_staking::Config>::BondingDuration::get(), 2);
		assert_eq!(<Runtime as plant_nomination_pools::Config>::MaxUnbonding::get(), 1);
		...
		assert_err!(
			...
			plant_staking::Error::<Runtime>::NoMoreChunks
		);

		assert_eq!(current_era(), 0);

		// progress over bonding duration.
		for _ in 0..=<Runtime as plant_staking::Config>::BondingDuration::get() {
		...
		assert_eq!(
			...
			[
				...
				plant_staking::Event::Withdrawn { stash: pool_bonded_account, amount: 10 },
				plant_staking::Event::Unbonded { stash: pool_bonded_account, amount: 10 }
			]
		);
		...
		assert_err!(
			...
			plant_nomination_pools::Error::<Runtime>::CannotWithdrawAny
		);
