...
impl From<subsoil::npos_elections::Error> for MinerError {
...
impl<T: MinerConfig> Miner<T> {
	...
	/// Convert a raw solution from [`subsoil::npos_elections::ElectionResult`] to [`RawSolution`], which
	...
	) -> Result<(SolutionOf<T>, ElectionScore, SolutionOrSnapshotSize, TrimmingStatus), MinerError>
	{
		...
		let sorted_assignments = {
			...
			subsoil::npos_elections::reduce(&mut staked);

			// Sort the assignments by reversed voter stake. This ensures that we can efficiently
			// truncate the list.
			staked.sort_by_key(
				|subsoil::npos_elections::StakedAssignment::<T::AccountId> { who, .. }| {
			...
			let _ = subsoil::npos_elections::to_supports(&staked)
				.iter_mut()
			...
			debug_assert!({
				...
				let expected_ok: Result<
					...
					_,
				> = subsoil::npos_elections::to_supports(&staked).try_into();
				expected_ok.is_ok()
			});
