...
use soil_test_staging_node_spec_builder::ChainSpecBuilder;
...
macro_rules! bash(
	( soil-test-staging-node-spec-builder $($a:tt)* ) => {{
		let bin_path = env!("CARGO_BIN_EXE_soil-test-staging-node-spec-builder");
		spawn_with_output!(
			$bin_path $($a)*
		)
