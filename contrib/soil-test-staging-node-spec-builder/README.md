...
See [`ChainSpecBuilderCmd`](https://docs.rs/soil-test-staging-node-spec-builder/6.0.0/soil_test_staging_node_spec_builder/enum.ChainSpecBuilderCmd.html)
for a list of available commands.

## Installation

```bash
cargo install soil-test-staging-node-spec-builder --locked
```
...
```bash
soil-test-staging-node-spec-builder -c "/dev/stdout" create --relay-chain "dev" --para-id 1000 -r $runtime_path named-preset "staging"
```
...
```rust,ignore
bash!(
	soil-test-staging-node-spec-builder -c "/dev/stdout" create -r $runtime_path default
)
```
...
```rust,ignore
bash!(
	soil-test-staging-node-spec-builder display-preset -r $runtime_path
)
```
...
fn cmd_display_preset(runtime_path: &str) -> String {
	bash!(
		soil-test-staging-node-spec-builder display-preset -r $runtime_path -p "staging"
	)
}
```
...
```rust,ignore
bash!(
	soil-test-staging-node-spec-builder list-presets -r $runtime_path
)
```
...
```rust,ignore
bash!(
	soil-test-staging-node-spec-builder -c "/dev/stdout" create --relay-chain "dev" -r $runtime_path named-preset "staging"
)
```
...
```rust,ignore
bash!(
	soil-test-staging-node-spec-builder -c "/dev/stdout" create -s -r $runtime_path patch "tests/input/patch.json"
)
```
...
```rust,ignore
bash!(
	soil-test-staging-node-spec-builder -c "/dev/stdout" create -s -r $runtime_path full "tests/input/full.json"
)
```
...
```rust,ignore
bash!(
	soil-test-staging-node-spec-builder -c "/dev/stdout" create -r $runtime_path patch "tests/input/patch.json"
)
```
...
```rust,ignore
bash!(
	soil-test-staging-node-spec-builder -c "/dev/stdout" create -r $runtime_path full "tests/input/full.json"
)
```
...
The `soil-test-staging-node-spec-builder` provides also some extra utilities: [`VerifyCmd`](https://docs.rs/soil-test-staging-node-spec-builder/latest/soil_test_staging_node_spec_builder/struct.VerifyCmd.html),
[`ConvertToRawCmd`](https://docs.rs/soil-test-staging-node-spec-builder/latest/soil_test_staging_node_spec_builder/struct.ConvertToRawCmd.html),
[`UpdateCodeCmd`](https://docs.rs/soil-test-staging-node-spec-builder/latest/soil_test_staging_node_spec_builder/struct.UpdateCodeCmd.html).
