...
Consequently, a runtime that wants to include this pallet must implement this trait."
	...
	// insert `topsoil_system::Config` supertrait with `RuntimeEvent: From<Event<Self>>` if neither
	...
	match &config.default_sub_trait {
		Some(default_sub_trait) if default_sub_trait.items.len() > 0 => {
			...
			quote!(
				...
				/// [`#[pallet::config(with_default)]`](`topsoil_support::pallet_macros::config`).
				/// Can be used in tandem with
				/// [`#[register_default_config]`](`topsoil_support::register_default_config`) and
				/// [`#[derive_impl]`](`topsoil_support::derive_impl`) to derive test config traits
				/// based on existing pallet config traits in a safe and developer-friendly way.
				///
				/// See [here](`topsoil_support::pallet_macros::config`) for more information and caveats about
...
pub fn expand_config_metadata(def: &Def) -> proc_macro2::TokenStream {
	...
	let types = def.config.associated_types_metadata.iter().map(|metadata| {
		...
		quote::quote_spanned!(span => {
			#( #cfgs ) *
			#topsoil_support::__private::metadata_ir::PalletAssociatedTypeMetadataIR {
				name: #ident_str,
				ty: #topsoil_support::__private::scale_info::meta_type::<
	...
	quote::quote!(
		impl<#type_impl_gen> #pallet_ident<#type_use_gen> #completed_where_clause {
			...
			pub fn pallet_associated_types_metadata()
				-> #topsoil_support::__private::vec::Vec<#topsoil_support::__private::metadata_ir::PalletAssociatedTypeMetadataIR>
			{
				#topsoil_support::__private::vec![ #( #types ),* ]
			}
		}
	)
}
