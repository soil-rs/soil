		...
		loop {
			...
			break match maybe_next {
				Some(next) => {
					...
					let maybe_value = topsoil_core::storage::unhashed::get::<T>(&next);
					match maybe_value {
						Some(value) => {
							if self.drain {
								topsoil_core::storage::unhashed::kill(&next);
							}
...
pub fn get_storage_value<T: Decode + Sized>(module: &[u8], item: &[u8], hash: &[u8]) -> Option<T> {
	...
	topsoil_core::storage::unhashed::get::<T>(&key)
}
...
pub fn take_storage_value<T: Decode + Sized>(module: &[u8], item: &[u8], hash: &[u8]) -> Option<T> {
	...
	key[0..32].copy_from_slice(&storage_prefix);
	key[32..].copy_from_slice(hash);
	topsoil_core::storage::unhashed::take::<T>(&key)
}
...
pub fn put_storage_value<T: Encode>(module: &[u8], item: &[u8], hash: &[u8], value: T) {
	...
	key[0..32].copy_from_slice(&storage_prefix);
	key[32..].copy_from_slice(hash);
	topsoil_core::storage::unhashed::put(&key, &value);
}
...
pub fn remove_storage_prefix(module: &[u8], item: &[u8], hash: &[u8]) {
	...
	key[0..32].copy_from_slice(&storage_prefix);
	key[32..].copy_from_slice(hash);
	let _ = topsoil_core::storage::unhashed::clear_prefix(&key, None, None);
}
...
pub fn clear_storage_prefix(
	...
	key[0..32].copy_from_slice(&storage_prefix);
	key[32..].copy_from_slice(hash);
	topsoil_core::storage::unhashed::clear_prefix(&key, maybe_limit, maybe_cursor)
}
...
/// ```
/// # use topsoil_core::storage::migration::move_storage_from_pallet;
/// # subsoil::io::TestExternalities::new_empty().execute_with(|| {
...
/// "my_new_example_name", a migration can be:
/// ```
/// # use topsoil_core::storage::migration::move_pallet;
/// # subsoil::io::TestExternalities::new_empty().execute_with(|| {
...
mod tests {
	...
	impl topsoil_core::traits::StorageInstance for OldPalletStorageValuePrefix {
	...
	impl topsoil_core::traits::StorageInstance for OldPalletStorageMapPrefix {
	...
	impl topsoil_core::traits::StorageInstance for NewPalletStorageValuePrefix {
		const STORAGE_PREFIX: &'static str = "foo_value";
		fn pallet_prefix() -> &'static str {
			"my_new_pallet"
		}
	...
	impl topsoil_core::traits::StorageInstance for NewPalletStorageMapPrefix {
		const STORAGE_PREFIX: &'static str = "foo_map";
		fn pallet_prefix() -> &'static str {
			"my_new_pallet"
		}
