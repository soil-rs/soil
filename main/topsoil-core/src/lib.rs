...
/// Export ourself as `topsoil_core` to make tests happy.
#[doc(hidden)]
extern crate self as topsoil_core;

#[doc(hidden)]
extern crate alloc;

/// System pallet (formerly the `topsoil-system` crate).
pub mod system;
...
/// This is useful for type generic over runtime:
/// ```
/// # use topsoil_core::{EqNoBound, PartialEqNoBound};
/// trait Config {
/// 		type C: Eq;
/// }
///
...
/// This is useful for type generic over runtime:
/// ```
/// # use topsoil_core::PartialEqNoBound;
/// trait Config {
/// 		type C: PartialEq;
/// }
///
...
/// This is useful for type generic over runtime:
/// ```
/// # use topsoil_core::{OrdNoBound, PartialOrdNoBound, EqNoBound, PartialEqNoBound};
/// trait Config {
/// 		type C: Ord;
/// }
///
...
/// This is useful for type generic over runtime:
/// ```
/// # use topsoil_core::DefaultNoBound;
...
/// ```
///
/// The documentation for each individual part can be found at [topsoil_core::pallet_macros]
///
...
///   [`MaxEncodedLen`](topsoil_core::pallet_prelude::MaxEncodedLen) on storage types. This is
...
/// `#[topsoil_core::pallet]` attribute macro that encloses your pallet module. This
...
/// * [`OnGenesis`](topsoil_core::traits::OnGenesis): contains some logic to write the pallet
///   version into storage.
/// * [`PalletInfoAccess`](topsoil_core::traits::PalletInfoAccess) to ease access to pallet
///   information given by [`topsoil_core::traits::PalletInfo`]. (The implementation uses the
///   associated type [`topsoil_core::traits::PalletInfo`]).
/// * [`StorageInfoTrait`](topsoil_core::traits::StorageInfoTrait) to give information about
///   storages.
///
/// If the attribute `set_storage_max_encoded_len` is set then the macro calls
/// [`StorageInfoTrait`](topsoil_core::traits::StorageInfoTrait) for each storage in the
/// implementation of [`StorageInfoTrait`](topsoil_core::traits::StorageInfoTrait) for the
/// pallet. Otherwise, it implements
/// [`StorageInfoTrait`](topsoil_core::traits::StorageInfoTrait) for the pallet using the
/// [`PartialStorageInfoTrait`](topsoil_core::traits::PartialStorageInfoTrait)
...
pub mod pallet_macros {
	...
	/// mod pallet {
	/// # 	use topsoil_core::pallet_prelude::*;
	/// # 	use topsoil_core::inherent::IsFatalError;
	...
	/// #
	/// # 	#[pallet::config]
	/// # 	pub trait Config: topsoil_core::system::Config<RuntimeEvent: From<Event<Self>>> {
	/// # 	}
	/// }
	/// ```
	///
	...
	///
	/// To bypass the syntactic `topsoil_core::system::Config` supertrait check, use the attribute
	/// `pallet::disable_frame_system_supertrait_check`.
	///
	/// Note this bypass is purely syntactic, and does not actually remove the requirement that your
	/// pallet implements `topsoil_core::system::Config`. When using this check, your config is still required to implement
	/// `topsoil_core::system::Config` either via
	/// - Implementing a trait that itself implements `topsoil_core::system::Config`
	/// - Tightly coupling it with another pallet which itself implements `topsoil_core::system::Config`
	///
	/// e.g.
	///
	/// ```
	...
	/// # 	use topsoil_core::pallet_prelude::*;
	/// # 	use topsoil_core::system::pallet_prelude::*;
	/// 	trait OtherTrait: topsoil_core::system::Config {}
	///
	/// 	#[pallet::pallet]
	/// 	pub struct Pallet<T>(_);
	///
	...
	///
	/// 	#[pallet::config]
	/// 	pub trait Config: topsoil_core::system::Config // + $optionally_some_other_supertraits
	...
	/// [`topsoil_core::system::pallet::Config`](../../topsoil_system/pallet/trait.Config.html), and
	...
	/// [`#[derive_impl(..)`](`topsoil_core::derive_impl`) to derive a default testing
	/// config:
	///
	/// ```
	/// #[topsoil_core::pallet]
	...
	/// # 	use topsoil_core::system::pallet_prelude::*;
	/// # 	use core::fmt::Debug;
	/// # 	use topsoil_core::traits::Contains;
	/// #
	...
	/// test `Config` is derived using the [`#[derive_impl(..)]`](`topsoil_core::derive_impl`)
	...
	///   with [`derive_impl`](`topsoil_core::derive_impl`).
	...
	/// # 	use topsoil_core::system::pallet_prelude::*;
	/// # 	use core::fmt::Debug;
	/// # 	use topsoil_core::traits::{Contains, VariantCount};
	/// #
	/// # 	pub trait SomeMoreComplexBound {}
	/// #
	/// 	#[pallet::pallet]
	...
	/// [`topsoil_core::construct_runtime`] will have the name of `RuntimeFreezeReason`,
	...
	/// [`ValidateUnsigned`](topsoil_core::pallet_prelude::ValidateUnsigned) for
	...
	/// by implementing the
	/// [`RuntimeViewFunction`](topsoil_core::view_functions::runtime_api::RuntimeViewFunction)
	...
	/// [`GetStorageVersion`](topsoil_core::traits::GetStorageVersion), the current storage
	...
	/// #[topsoil_core::pallet]
	/// mod pallet {
	/// # 	use topsoil_core::pallet_prelude::StorageVersion;
	/// # 	use topsoil_core::traits::GetStorageVersion;
	/// #
	...
	/// [`OnInitialize`](topsoil_core::traits::OnInitialize),
	/// [`OnIdle`](topsoil_core::traits::OnIdle),
	/// [`OnFinalize`](topsoil_core::traits::OnFinalize),
	/// [`OnRuntimeUpgrade`](topsoil_core::traits::OnRuntimeUpgrade),
	/// [`OffchainWorker`](topsoil_core::traits::OffchainWorker), and
	/// [`IntegrityTest`](topsoil_core::traits::IntegrityTest) using
	/// the provided [`Hooks`](topsoil_core::traits::Hooks) implementation.
	///
	...
	/// * `#[derive(`[`topsoil_core::CloneNoBound`]`)]`
	/// * `#[derive(`[`topsoil_core::EqNoBound`]`)]`
	/// * `#[derive(`[`topsoil_core::PartialEqNoBound`]`)]`
	...
	/// extension such as `topsoil_core::system::CheckNonce` is used.
	/// Extensions such as `topsoil_core::system::CheckNonce` require a funded account to validate
	/// the transaction.
	///
	/// ### Macro expansion
	///
	...
	/// [`topsoil_core::MAX_MODULE_ERROR_ENCODED_SIZE`].
	///
	...
	///
	/// Field types in enum variants must also implement [`topsoil_core::PalletError`],
	/// otherwise the pallet will fail to compile. Rust primitive types have already
	/// implemented the [`topsoil_core::PalletError`] trait along with some commonly used
	...
	/// #[topsoil_core::pallet(dev_mode)]
	/// mod pallet {
	/// #     use topsoil_core::pallet_prelude::IsType;
	/// #
	/// 	#[pallet::pallet]
	/// 	pub struct Pallet<T>(_);
	///
	...
	/// `Member`, available in [`topsoil_core::pallet_prelude`].
	///
	...
	/// [`topsoil_core::storage::with_storage_layer`] to make it transactional. Thus if the
	...
	///
	/// fn main() {
	/// #   use topsoil_core::{derive_impl, construct_runtime};
	/// #   use topsoil_core::__private::codec::Encode;
	/// #   use topsoil_core::__private::TestExternalities;
	...
	///     let origin: RuntimeOrigin = topsoil_core::system::RawOrigin::Signed(10).into();
	...
	/// #     #[pallet::pallet]
	/// #     pub struct Pallet<T>(_);
	/// #     use topsoil_core::traits::GenesisBuild;
	///     #[pallet::genesis_config]
	///     #[derive(topsoil_core::DefaultNoBound)]
	///     pub struct GenesisConfig<T: Config> {
	/// 		foo: Vec<T::AccountId>
	...
	/// [`StorageDoubleMap`](topsoil_core::storage::types::StorageDoubleMap) needs two of
	/// each, and [`StorageNMap`](topsoil_core::storage::types::StorageNMap) needs `N` such
	/// pairs. Since [`StorageValue`](topsoil_core::storage::types::StorageValue) only stores
	...
	/// [`ValueQuery`](topsoil_core::storage::types::ValueQuery) is used, explained in the
	/// next section.
	///
	/// ### QueryKind
	///
	/// Every storage type mentioned above has a generic type called
	/// [`QueryKind`](topsoil_core::storage::types::QueryKindTrait) that determines its
	/// "query" type. This refers to the kind of value returned when querying the storage, for
	/// instance, through a `::get()` method.
	///
	/// There are three types of queries:
	///
	/// 1. [`OptionQuery`](topsoil_core::storage::types::OptionQuery): The default query type.
	///    It returns `Some(V)` if the value is present, or `None` if it isn't, where `V` is
	///    the value type.
	/// 2. [`ValueQuery`](topsoil_core::storage::types::ValueQuery): Returns the value itself
	...
	///    GetDefault`.
	/// 3. [`ResultQuery`](topsoil_core::storage::types::ResultQuery): Returns `Result<V, E>`,
	///    where `V` is the value type.
	///
	/// See [`QueryKind`](topsoil_core::storage::types::QueryKindTrait) for further examples.
	///
	/// ### Optimized Appending
	///
	/// All storage items — such as
	/// [`StorageValue`](topsoil_core::storage::types::StorageValue),
	/// [`StorageMap`](topsoil_core::storage::types::StorageMap), and their variants—offer an
	...
	/// [`BoundedVec`](topsoil_core::BoundedVec).
	///
	/// #### Example
	...
	/// All storage items — such as
	/// [`StorageValue`](topsoil_core::storage::types::StorageValue),
	/// [`StorageMap`](topsoil_core::storage::types::StorageMap), and their counterparts —
	...
	/// [`StorageValue`](topsoil_core::storage::types::StorageValue), a set of hashers needs
	...
	/// [`topsoil_core::StorageHasher`](topsoil_core::StorageHasher).
	///
	/// Lastly, it's recommended for hashers with "concat" to have reversible hashes. Refer to
	/// the implementors section of
	/// [`hash::ReversibleStorageHasher`](topsoil_core::hash::ReversibleStorageHasher).
	///
	/// ### Prefixes
	///
	...
	/// > [`topsoil_core::construct_runtime`](topsoil_core::construct_runtime), and
	/// > `STORAGE_PREFIX` is the name of the `type` aliased to a particular storage type, such
	/// > as
	/// > `Foo` in `type Foo<T> = StorageValue<..>`.
	///
	/// For [`StorageValue`](topsoil_core::storage::types::StorageValue), no additional key is
