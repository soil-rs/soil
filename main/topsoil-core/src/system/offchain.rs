...
//! ## Usage
//!
//! Please refer to [`example-offchain-worker`](../../plant_example_offchain_worker/index.html) for
...
//! See [`PricePayload`](../../plant_example_offchain_worker/struct.PricePayload.html)
//!
...
pub trait SigningTypes: crate::system::Config {
...
/// extension: [`crate::system::AuthorizeCall`].
/// The authorization logic is defined on the call with the attribute:
/// [`topsoil_core::pallet_macros::authorize`].
///
...
pub trait CreateAuthorizedTransaction<LocalCall>: CreateTransaction<LocalCall> {
	...
	///
	/// For more information about authorized call see [`topsoil_core::pallet_prelude::authorize`].
...
>
{
	...
	fn send_single_signed_transaction(
		...
		let mut account_data = crate::system::Account::<T>::get(&account.id);
		log::debug!(
		...
		if res.is_ok() {
			...
			crate::system::Account::<T>::insert(&account.id, account_data);
		}
...
mod tests {
	use super::*;
	use crate::system::mock::{RuntimeCall, Test as TestRuntime, CALL};
	use codec::Decode;
