...
use topsoil_core::traits::{Authorize, PostInherents, PostTransactions, PreInherents};
...
#[topsoil_core::pallet]
pub mod pallet {
	use crate::system::{self as topsoil_system, pallet_prelude::*, *};
	...
	pub mod config_preludes {
		...
		use topsoil_core::{derive_impl, traits::Get};
		...
		/// [`derive_impl`](`topsoil_core::derive_impl`) to derive a testing pallet config
		/// based on this one.
		///
		...
		impl DefaultConfig for TestDefaultConfig {
			...
			type BlockHashCount = TestBlockHashCount<topsoil_core::traits::ConstU32<10>>;
			type OnSetCode = ();
		...
		///   if you use `plant-balances` or similar.
		...
		#[topsoil_core::register_default_impl(SolochainDefaultConfig)]
		impl DefaultConfig for SolochainDefaultConfig {
			...
			type MaxConsumers = topsoil_core::traits::ConstU32<128>;
			...
			type BlockHashCount = TestBlockHashCount<topsoil_core::traits::ConstU32<256>>;
		...
		#[topsoil_core::register_default_impl(RelayChainDefaultConfig)]
		...
		/// It currently uses the same configuration as `SolochainDefaultConfig`.
		#[derive_impl(SolochainDefaultConfig as DefaultConfig, no_aggregated_types)]
		#[topsoil_core::register_default_impl(ParaChainDefaultConfig)]
	...
	pub trait Config: 'static + Eq + Clone {
		...
		/// [`topsoil_core::traits::InsideBoth`], [`topsoil_core::traits::TheseExcept`] or
		/// [`topsoil_core::traits::EverythingBut`] et al. The default would be
		/// [`topsoil_core::traits::Everything`].
		#[pallet::no_default_bounds]
...
impl<T: Config> Pallet<T> {
	...
	/// runtime migrations
	/// [`on_runtime_upgrade`](topsoil_core::traits::OnRuntimeUpgrade::on_runtime_upgrade)
	...
	///
	/// Example:
	#[doc = docify::embed!("src/system/tests.rs", last_runtime_upgrade_spec_version_usage)]
	...
	pub fn run_to_block_with<AllPalletsWithSystem>(
		...
		AllPalletsWithSystem: topsoil_core::traits::OnInitialize<BlockNumberFor<T>>
			+ topsoil_core::traits::OnFinalize<BlockNumberFor<T>>,
	{
	...
	{
		let already_reclaimed = crate::system::ExtrinsicWeightReclaimed::<T>::get();
		...
		if to_reclaim_more != Weight::zero() {
			crate::system::BlockWeight::<T>::mutate(|current_weight| {
				current_weight.reduce(to_reclaim_more, info.class);
			});
			crate::system::ExtrinsicWeightReclaimed::<T>::put(accurate_reclaim);
		}
...
pub mod pallet_prelude {
	...
	pub type OriginFor<T> = <T as crate::system::Config>::RuntimeOrigin;

	/// Type alias for the `Header`.
	pub type HeaderFor<T> =
		<<T as crate::system::Config>::Block as subsoil::runtime::traits::HeaderProvider>::HeaderT;
	...
	pub type ExtrinsicFor<T> =
		<<T as crate::system::Config>::Block as subsoil::runtime::traits::Block>::Extrinsic;

	/// Type alias for the `RuntimeCall` associated type of system config.
	pub type RuntimeCallFor<T> = <T as crate::system::Config>::RuntimeCall;

	/// Type alias for the `AccountId` associated type of system config.
	pub type AccountIdFor<T> = <T as crate::system::Config>::AccountId;
}
