...
use topsoil_core_procedural_tools::{
	...
	if !has_is_type_bound {
		let msg =
			"Invalid `type RuntimeEvent`, associated type `RuntimeEvent` is reserved and must \
					bound: `IsType<<Self as topsoil_core::system::Config>::RuntimeEvent>`"
				.to_string();
...
fn has_expected_system_config(path: syn::Path, topsoil_system: &syn::Path) -> bool {
	...
	// the parse path might be something like `topsoil_core::system::Config<...>`, so we
...
impl ConfigDef {
	pub fn try_from(
		...
		for trait_item in &mut item.items {
			// Skip RuntimeEvent bound check for the system pallet itself — it defines RuntimeEvent,
			// so the `IsType<<Self as system::Config>::RuntimeEvent>` bound check is self-referential.
			let is_event = if is_frame_system {
				matches!(trait_item, syn::TraitItem::Type(t) if t.ident == "RuntimeEvent")
			} else {
				check_event_type(topsoil_system, trait_item, has_instance)?
			};
		...
		if !has_frame_system_supertrait && !disable_system_supertrait_check {
			...
			let msg = format!(
				...
				found {}. \
				(try `pub trait Config: topsoil_core::system::Config {{ ...` or \
...
mod tests {
	...
	fn has_expected_system_config_works() {
		...
		let path =
			syn::parse2::<syn::Path>(quote::quote!(topsoil_core::system::Config)).unwrap();
	...
	fn has_expected_system_config_works_with_assoc_type() {
		let topsoil_system =
			syn::parse2::<syn::Path>(quote::quote!(topsoil_core::system)).unwrap();
		let path =
			syn::parse2::<syn::Path>(quote::quote!(topsoil_core::system::Config<RuntimeCall = Call>))
				.unwrap();
		assert!(has_expected_system_config(path, &topsoil_system));
	}
	...
	fn has_expected_system_config_works_with_frame() {
		...
		let topsoil_system =
			syn::parse2::<syn::Path>(quote::quote!(topsoil::deps::topsoil_core::system)).unwrap();
	...
	fn has_expected_system_config_works_with_frame_full_path() {
		let topsoil_system =
			syn::parse2::<syn::Path>(quote::quote!(topsoil::deps::topsoil_core::system)).unwrap();
		let path =
			syn::parse2::<syn::Path>(quote::quote!(topsoil::deps::topsoil_core::system::Config)).unwrap();
		assert!(has_expected_system_config(path, &topsoil_system));
	}
	...
	fn has_expected_system_config_works_with_other_frame_full_path() {
		let topsoil_system =
			syn::parse2::<syn::Path>(quote::quote!(topsoil::xyz::topsoil_core::system)).unwrap();
		let path =
			syn::parse2::<syn::Path>(quote::quote!(topsoil::xyz::topsoil_core::system::Config)).unwrap();
		assert!(has_expected_system_config(path, &topsoil_system));
	}
	...
	fn has_expected_system_config_unexpected_frame_system() {
		...
		let path = syn::parse2::<syn::Path>(quote::quote!(topsoil_core::system::Config)).unwrap();
		assert!(!has_expected_system_config(path, &topsoil_system));
	}
	...
	fn has_expected_system_config_unexpected_path() {
		let topsoil_system = syn::parse2::<syn::Path>(quote::quote!(topsoil_system)).unwrap();
		let path = syn::parse2::<syn::Path>(quote::quote!(topsoil_core::system::ConfigSystem)).unwrap();
		assert!(!has_expected_system_config(path, &topsoil_system));
	}
