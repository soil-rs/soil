...
fn expand_weight(
	...
	match weight {
		CallWeightDef::DevModeDefault => quote::quote!(
			#topsoil_core::pallet_prelude::Weight::zero()
		),
...
pub fn expand_call(def: &mut Def) -> proc_macro2::TokenStream {
			...
			if let syn::ImplItem::Fn(method) = item {
				...
				let (ok_type, err_type) = match return_type {
					CallReturnType::DispatchResult => (
						quote::quote!(()),
						quote::quote!(#topsoil_core::pallet_prelude::DispatchError),
					),
					CallReturnType::DispatchResultWithPostInfo => (
						quote::quote!(#topsoil_core::dispatch::PostDispatchInfo),
						quote::quote!(#topsoil_core::dispatch::DispatchErrorWithPostInfo),
					),
				};
				...
				method.block = syn::parse_quote! {{
					...
					#topsoil_core::storage::with_storage_layer::<#ok_type, #err_type, _>(
						|| #block
					)
				}};
			}
				...
				let authorize_fn_pallet_impl = quote::quote_spanned!(authorize_fn.span() =>
					...
					impl<#type_impl_gen> Pallet<#type_use_gen> #where_clause {
						...
						fn #attr_fn_getter() -> impl Fn(
							#topsoil_core::pallet_prelude::TransactionSource,
							#( &#arg_type ),*
						) -> #topsoil_core::pallet_prelude::TransactionValidityWithRefund {
							#authorize_fn
						}
					}
				);
	...
	for method in &methods {
		let w = match &method.authorize {
			...
			None => quote::quote!(#topsoil_core::pallet_prelude::Weight::zero()),
		};
	...
	quote::quote_spanned!(span =>
		...
		impl<#type_impl_gen> #topsoil_core::dispatch::GetDispatchInfo
			for #call_ident<#type_use_gen>
			#where_clause
		{
			fn get_dispatch_info(&self) -> #topsoil_core::dispatch::DispatchInfo {
				match *self {
					#(
						#cfg_attrs
						Self::#fn_name { #( #args_name_pattern_ref, )* } => {
							...
							let __pallet_weight = <
								dyn #topsoil_core::dispatch::WeighData<( #( & #args_type, )* )>
							>::weigh_data(&__pallet_base_weight, ( #( #args_name, )* ));

							let __pallet_class = <
								dyn #topsoil_core::dispatch::ClassifyDispatch<
									( #( & #args_type, )* )
								>
							>::classify_dispatch(&__pallet_base_weight, ( #( #args_name, )* ));

							let __pallet_pays_fee = <
								dyn #topsoil_core::dispatch::PaysFee<( #( & #args_type, )* )>
							>::pays_fee(&__pallet_base_weight, ( #( #args_name, )* ));

							#topsoil_core::dispatch::DispatchInfo {
		...
		impl<#type_impl_gen> #topsoil_core::dispatch::CheckIfFeeless for #call_ident<#type_use_gen>
			#where_clause
		{
		...
		impl<#type_impl_gen> #topsoil_core::traits::GetCallName for #call_ident<#type_use_gen>
			#where_clause
		{
		...
		impl<#type_impl_gen> #topsoil_core::traits::GetCallIndex for #call_ident<#type_use_gen>
			#where_clause
		{
		...
		impl<#type_impl_gen> #topsoil_core::traits::UnfilteredDispatchable
			for #call_ident<#type_use_gen>
			#where_clause
		{
			...
			) -> #topsoil_core::dispatch::DispatchResultWithPostInfo {
				#topsoil_core::dispatch_context::run_in_context(|| {
					match self {
						#(
							#cfg_attrs
							Self::#fn_name { #( #args_name_pattern, )* } => {
								#topsoil_core::__private::subsoil::enter_span!(
									#topsoil_core::__private::subsoil::tracing::trace_span!(stringify!(#fn_name))
								);
								#maybe_allow_attrs
		...
		impl<#type_impl_gen> #topsoil_core::dispatch::Callable<T> for #pallet_ident<#type_use_gen>
			#where_clause
		{
		...
		impl<#type_impl_gen> #pallet_ident<#type_use_gen> #where_clause {
			#[allow(dead_code)]
			#[doc(hidden)]
			pub fn call_functions() -> #topsoil_core::__private::metadata_ir::PalletCallMetadataIR {
				#topsoil_core::__private::metadata_ir::PalletCallMetadataIR  {
					ty: #topsoil_core::__private::scale_info::meta_type::<#call_ident<#type_use_gen>>(),
		...
		impl<#type_impl_gen> #topsoil_core::traits::Authorize for #call_ident<#type_use_gen>
			#where_clause
		{
			fn authorize(&self, source: #topsoil_core::pallet_prelude::TransactionSource) -> ::core::option::Option<::core::result::Result<
				(
					#topsoil_core::pallet_prelude::ValidTransaction,
					#topsoil_core::pallet_prelude::Weight,
				),
				#topsoil_core::pallet_prelude::TransactionValidityError
			>>
			{
				match *self {
					#(
			...
			fn weight_of_authorize(&self) -> #topsoil_core::pallet_prelude::Weight {
				match *self {
					#(
						#cfg_attrs
						Self::#fn_name { #( #args_name_pattern_ref, )* } => {
