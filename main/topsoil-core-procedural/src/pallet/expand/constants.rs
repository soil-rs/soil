...
pub fn expand_constants(def: &mut Def) -> proc_macro2::TokenStream {
		...
		config_consts.push(ConstDef {
			...
			default_byte_impl: quote::quote!(
				...
				let value = <<T as Config #trait_use_gen>::#ident as
					#topsoil_core::traits::Get<#const_type>>::get();
				#topsoil_core::__private::codec::Encode::encode(&value)
			),
			metadata_name: None,
			deprecation_info,
		})
	...
	let consts = config_consts.into_iter().chain(extra_consts.into_iter()).map(|const_| {
		...
		quote::quote!({
			#topsoil_core::__private::metadata_ir::PalletConstantMetadataIR {
				name: #ident_str,
				ty: #topsoil_core::__private::scale_info::meta_type::<#const_type>(),
				value: { #default_byte_impl },
				docs: #topsoil_core::__private::vec![ #( #doc ),* ],
	...
	quote::quote!(
		impl<#type_impl_gen> #pallet_ident<#type_use_gen> #completed_where_clause{
			...
			pub fn pallet_constants_metadata()
				-> #topsoil_core::__private::Vec<#topsoil_core::__private::metadata_ir::PalletConstantMetadataIR>
			{
				#topsoil_core::__private::vec![ #( #consts ),* ]
			}
		}
	)
}
