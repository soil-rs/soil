...
pub fn process_generics(def: &mut Def) -> syn::Result<Vec<ResultOnEmptyStructMetadata>> {
	...
	for storage_def in def.storages.iter_mut() {
		...
		let default_query_kind: syn::Type =
			syn::parse_quote!(#topsoil_core::storage::types::OptionQuery);
		let mut default_on_empty = |value_ty: syn::Type| -> syn::Type {
			...
			syn::parse_quote!(#topsoil_core::traits::GetDefault)
		};
		let default_max_values: syn::Type = syn::parse_quote!(#topsoil_core::traits::GetDefault);
			...
			if storage_def.use_default_hasher {
				...
				for hasher_idx in hasher_indices {
					args.args[hasher_idx] = syn::GenericArgument::Type(
						syn::parse_quote!(#topsoil_core::Blake2_128Concat),
					);
				}
			}
...
pub fn expand_storages(def: &mut Def) -> proc_macro2::TokenStream {
	...
	for storage in def.storages.iter() {
		...
		entries_builder.push(quote::quote_spanned!(storage.attr_span =>
			#(#cfg_attrs)*
			#(#maybe_allow_attrs)*
			(|entries: &mut #topsoil_core::__private::Vec<_>| {
				{
					<#full_ident as #topsoil_core::storage::StorageEntryMetadataBuilder>::build_metadata(
						#deprecation,
						#topsoil_core::__private::vec![
							#( #docs, )*
						],
						entries,
					);
				...
				Metadata::Value { value } => {
					...
					quote::quote_spanned!(storage.attr_span =>
						...
						impl<#type_impl_gen> #pallet_ident<#type_use_gen> #completed_where_clause {
							...
							pub fn #getter() -> #query {
								<
									#full_ident as #topsoil_core::storage::StorageValue<#value>
								>::get()
							}
						}
					)
					...
					quote::quote_spanned!(storage.attr_span =>
						...
						impl<#type_impl_gen> #pallet_ident<#type_use_gen> #completed_where_clause {
							...
							pub fn #getter<KArg>(k: KArg) -> #query where
								KArg: #topsoil_core::__private::codec::EncodeLike<#key>,
							{
								<
									#full_ident as #topsoil_core::storage::StorageMap<#key, #value>
								>::get(k)
							}
						}
					)
					...
					quote::quote_spanned!(storage.attr_span =>
						...
						impl<#type_impl_gen> #pallet_ident<#type_use_gen> #completed_where_clause {
							...
							pub fn #getter<KArg1, KArg2>(k1: KArg1, k2: KArg2) -> #query where
								KArg1: #topsoil_core::__private::codec::EncodeLike<#key1>,
								KArg2: #topsoil_core::__private::codec::EncodeLike<#key2>,
							{
								<
									#full_ident as
									#topsoil_core::storage::StorageDoubleMap<#key1, #key2, #value>
								>::get(k1, k2)
							}
						}
					)
				...
				Metadata::NMap { keygen, value, .. } => {
					...
					quote::quote_spanned!(storage.attr_span =>
						...
						impl<#type_impl_gen> #pallet_ident<#type_use_gen> #completed_where_clause {
							...
							where
								KArg: #topsoil_core::storage::types::EncodeLikeTuple<
									<#keygen as #topsoil_core::storage::types::KeyGenerator>::KArg
								>
									+ #topsoil_core::storage::types::TupleToEncodedIter,
							{
								<
									#full_ident as
									#topsoil_core::storage::StorageNMap<#keygen, #value>
								>::get(key)
							}
						}
					)
		...
		let maybe_counter = match storage_def.metadata {
			Metadata::CountedMap { .. } => {
				...
				quote::quote_spanned!(storage_def.attr_span =>
					...
					#(#cfg_attrs)*
					impl<#type_impl_gen> #topsoil_core::traits::StorageInstance
					...
					#(#cfg_attrs)*
					impl<#type_impl_gen> #topsoil_core::storage::types::CountedStorageMapInstance
			...
			Metadata::CountedNMap { .. } => {
				...
				quote::quote_spanned!(storage_def.attr_span =>
					...
					#(#cfg_attrs)*
					impl<#type_impl_gen> #topsoil_core::storage::types::CountedStorageNMapInstance
						for #prefix_struct_ident<#type_use_gen>
						#config_where_clause
					{
						type CounterPrefix = #counter_prefix_struct_ident<#type_use_gen>;
	...
	let on_empty_structs = on_empty_struct_metadata.into_iter().map(|metadata| {
		...
		quote::quote_spanned!(span =>
			...
			impl<#type_impl_gen> #topsoil_core::traits::Get<Result<#value_ty, #error_path>>
				for #name
				#config_where_clause
			{
				#[allow(deprecated)]
	...
	let try_decode_entire_state = {
		...
		quote::quote!(
			#topsoil_core::try_runtime_enabled! {
				#[allow(deprecated)]
				impl<#type_impl_gen> #topsoil_core::traits::TryDecodeEntireStorage
				for #pallet_ident<#type_use_gen> #completed_where_clause
				{
					fn try_decode_entire_state() -> Result<usize, #topsoil_core::__private::Vec<#topsoil_core::traits::TryDecodeEntireStorageError>> {
						let pallet_name = <<T as #topsoil_system::Config>::PalletInfo	as #topsoil_core::traits::PalletInfo>
							::name::<#pallet_ident<#type_use_gen>>()
							.expect("Every active pallet has a name in the runtime; qed");

						#topsoil_core::__private::log::debug!(target: "runtime::try-decode-state", "trying to decode pallet: {pallet_name}");

						// NOTE: for now, we have to exclude storage items that are feature gated.
						let mut errors = #topsoil_core::__private::Vec::new();
						let mut decoded = 0usize;

						#(
							#topsoil_core::__private::log::debug!(target: "runtime::try-decode-state", "trying to decode storage: \
							{pallet_name}::{}", stringify!(#storage_names));

							match <#storage_names as #topsoil_core::traits::TryDecodeEntireStorage>::try_decode_entire_state() {
								Ok(count) => {
									decoded += count;
								},
								Err(err) => {
	...
	quote::quote!(
		...
		{
			#[doc(hidden)]
			pub fn storage_metadata() -> #topsoil_core::__private::metadata_ir::PalletStorageMetadataIR {
				#topsoil_core::__private::metadata_ir::PalletStorageMetadataIR {
					prefix: <
					...
					entries: {
						#[allow(unused_mut)]
						let mut entries = #topsoil_core::__private::vec![];
