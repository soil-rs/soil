		...
		if let Some(attr) = &self.attr {
			...
			quote! {
				...
				#[derive(
					...
					#topsoil_core::pallet_prelude::Encode,
					#topsoil_core::pallet_prelude::Decode,
					#topsoil_core::pallet_prelude::DecodeWithMemTracking,
					#topsoil_core::pallet_prelude::TypeInfo,
				)]
				#[codec(encode_bound())]
				#[codec(decode_bound())]
				...
				#vis enum #ident #generics {
					...
					#[doc(hidden)]
					#[codec(skip)]
					__Ignore(core::marker::PhantomData<(#type_use_generics)>, #topsoil_core::Never),
				}
...
impl TasksDef {
	fn expand_to_tokens(&self, def: &Def) -> TokenStream2 {
		...
		quote! {
			...
			impl #impl_generics #topsoil_core::traits::Task for #enum_use
			{
				type Enumeration = #topsoil_core::__private::IntoIter<#enum_use>;

				fn iter() -> Self::Enumeration {
					let mut all_tasks = #topsoil_core::__private::vec![];
					#(all_tasks
						.extend(#task_iters.map(|(#(#task_arg_names),*)| #enum_ident::#task_fn_idents { #(#task_arg_names: #task_arg_names.clone()),* })
						.collect::<#topsoil_core::__private::Vec<_>>());
					)*
					all_tasks.into_iter()
				}
				...
				fn run(&self) -> Result<(), #topsoil_core::pallet_prelude::DispatchError> {
					match self.clone() {
				...
				fn weight(&self) -> #topsoil_core::pallet_prelude::Weight {
					match self.clone() {
