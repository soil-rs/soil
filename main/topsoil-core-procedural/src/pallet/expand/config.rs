...
Consequently, a runtime that wants to include this pallet must implement this trait."
	...
	// insert `topsoil_core::system::Config` supertrait with `RuntimeEvent: From<Event<Self>>` if neither
	...
	if let Some(event) = &def.event {
		if !def.is_frame_system {
			...
			let supertrait_with_event_bound = syn::parse2::<syn::TypeParamBound>(
				quote! { #topsoil_system::Config<RuntimeEvent: From<Event<#event_use_gen>>> },
			)
	...
	match &config.default_sub_trait {
		Some(default_sub_trait) if default_sub_trait.items.len() > 0 => {
			...
			quote!(
				...
				/// [`#[pallet::config(with_default)]`](`topsoil_core::pallet_macros::config`).
				/// Can be used in tandem with
				/// [`#[register_default_config]`](`topsoil_core::register_default_config`) and
				/// [`#[derive_impl]`](`topsoil_core::derive_impl`) to derive test config traits
				/// based on existing pallet config traits in a safe and developer-friendly way.
				///
				/// See [here](`topsoil_core::pallet_macros::config`) for more information and caveats about
...
pub fn expand_config_metadata(def: &Def) -> proc_macro2::TokenStream {
	...
	let types = def.config.associated_types_metadata.iter().map(|metadata| {
		...
		quote::quote_spanned!(span => {
			#( #cfgs ) *
			#topsoil_core::__private::metadata_ir::PalletAssociatedTypeMetadataIR {
				name: #ident_str,
				ty: #topsoil_core::__private::scale_info::meta_type::<
	...
	quote::quote!(
		impl<#type_impl_gen> #pallet_ident<#type_use_gen> #completed_where_clause {
			...
			pub fn pallet_associated_types_metadata()
				-> #topsoil_core::__private::vec::Vec<#topsoil_core::__private::metadata_ir::PalletAssociatedTypeMetadataIR>
			{
				#topsoil_core::__private::vec![ #( #types ),* ]
			}
		}
	)
}
