...
//! 	pub type Balances = plant_balances; // Implicit definition of parts
//! }
//! ```
//! This call has some implicit pallet parts, thus it will expand to:
//! ```ignore
//! topsoil_core::__private::tt_call! {
//! 	macro = [{ plant_balances::tt_default_parts_v2 }]
//! 	~~> topsoil_core::match_and_insert! {
//! 		target = [{
//! 			topsoil_core::__private::tt_call! {
//! 				macro = [{ topsoil_core::system::tt_default_parts_v2 }]
//! 				~~> topsoil_core::match_and_insert! {
//! 					target = [{
//!                         #[topsoil_core::runtime]
//!                         mod runtime {
...
//!
//!             #[runtime::pallet_index(1)]
//! 		    pub type Balances = plant_balances + Pallet + Call; // Explicit definition of parts
//!         }
//! 	}]
//! 	pattern = [{ System = topsoil_system }]
//! 	tokens = [{ ::{+ Pallet + Call} }]
...
//!
//!     #[runtime::pallet_index(1)]
//! 	pub type Balances = plant_balances + Pallet + Call;
//! }
//! ```
//!
...
//! ```
//!
//! Visualizing the entire flow of `#[topsoil_core::runtime]`, it would look like the following:
//!
//! ```ignore
//! +----------------------+     +------------------------+     +-------------------+
//! |                      |     | (defined in pallet)    |     |                   |
	...
	if !attr.is_empty() {
		if let Ok(_) = syn::parse::<keyword::legacy_ordering>(attr.clone()) {
			...
			let msg = "Invalid runtime macro call: unexpected attribute. Macro call must be \
				bare, such as `#[topsoil_core::runtime]` or `#[runtime]`, or must specify the \
				`legacy_ordering` attribute, such as `#[topsoil_core::runtime(legacy_ordering)]` or \
