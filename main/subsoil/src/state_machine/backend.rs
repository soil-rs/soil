...
/// Whether to take `:pending_code` into account
...
#[cfg(feature = "std")]
impl From<crate::core::traits::CallContext> for TryPendingCode {
	fn from(context: crate::core::traits::CallContext) -> Self {
		match context {
			crate::core::traits::CallContext::Onchain => TryPendingCode::Yes,
			crate::core::traits::CallContext::Offchain => TryPendingCode::No,
		}
	}
}
	...
	fn fetch_runtime_code(&self) -> Option<std::borrow::Cow<'_, [u8]>> {
		if matches!(self.try_pending_code, TryPendingCode::Yes) {
			let pending_code = self
				.backend
				.storage(crate::core::storage::well_known_keys::PENDING_CODE)
				.ok()
				.flatten()
				.map(Into::into);
	...
	pub fn runtime_code(&self) -> Result<RuntimeCode<'_>, &'static str> {
		let maybe_pending_code_hash = match self.try_pending_code {
			...
			TryPendingCode::Yes => self
				.backend
				.storage_hash(crate::core::storage::well_known_keys::PENDING_CODE)
				.ok()
				.flatten(),
		};
