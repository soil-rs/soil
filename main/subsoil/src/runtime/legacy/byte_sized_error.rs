...
use crate::runtime::{ArithmeticError, TokenError};
...
pub type ApplyExtrinsicResult =
	Result<DispatchOutcome, crate::runtime::transaction_validity::TransactionValidityError>;

/// Convert the legacy `ApplyExtrinsicResult` type to the latest version.
pub fn convert_to_latest(old: ApplyExtrinsicResult) -> crate::runtime::ApplyExtrinsicResult {
	old.map(|outcome| {
		outcome.map_err(|e| match e {
			...
			DispatchError::Token(err) => crate::runtime::DispatchError::Token(err),
			DispatchError::Arithmetic(err) => crate::runtime::DispatchError::Arithmetic(err),
		})
	})
}
