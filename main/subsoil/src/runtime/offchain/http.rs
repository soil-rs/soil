...
//! Example:
//! ```rust,no_run
//! use subsoil::runtime::offchain::http::Request;
//!
...
impl<'a, I: AsRef<[u8]>, T: IntoIterator<Item = I>> Request<'a, T> {
	...
	pub fn send(self) -> Result<PendingRequest, HttpError> {
		...
		let id = crate::io::offchain::http_request_start(self.method.as_ref(), self.url, meta)
			.map_err(|_| HttpError::IoError)?;

		// add custom headers
		for header in &self.headers {
			crate::io::offchain::http_request_add_header(id, header.name(), header.value())
				.map_err(|_| HttpError::IoError)?
		}

		// write body
		for chunk in self.body {
			crate::io::offchain::http_request_write_body(id, chunk.as_ref(), self.deadline)?;
		}

		// finalize the request
		crate::io::offchain::http_request_write_body(id, &[], self.deadline)?;
...
impl PendingRequest {
	...
	pub fn try_wait_all(
		...
		let statuses = crate::io::offchain::http_response_wait(&ids, deadline.into());
...
impl Iterator for ResponseBody {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		if self.filled_up_to.is_none() {
			let result = crate::io::offchain::http_response_read_body(
				self.id,
				&mut self.buffer,
				self.deadline,
			);
...
mod tests {
	use super::*;
	use crate::core::offchain::{testing, OffchainWorkerExt};
