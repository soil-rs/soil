...
use crate::core::offchain::StorageKind;
...
impl<'a> StorageValueRef<'a> {
	...
	pub fn set(&self, value: &impl codec::Encode) {
		value.using_encoded(|val| crate::io::offchain::local_storage_set(self.kind, self.key, val))
	}

	/// Remove the associated value from the storage.
	pub fn clear(&mut self) {
		crate::io::offchain::local_storage_clear(self.kind, self.key)
	}
	...
	pub fn get<T: codec::Decode>(&self) -> Result<Option<T>, StorageRetrievalError> {
		crate::io::offchain::local_storage_get(self.kind, self.key)
	...
	{
		let value = crate::io::offchain::local_storage_get(self.kind, self.key);
		let decoded = value
			.as_deref()
		...
		let set = val.using_encoded(|new_val| {
			crate::io::offchain::local_storage_compare_and_set(self.kind, self.key, value, new_val)
		});
		if set {
			Ok(val)
		} else {
...
mod tests {
	use super::*;
	use crate::core::offchain::{testing, OffchainDbExt};
