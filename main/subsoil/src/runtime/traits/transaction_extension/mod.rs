	...
	Codec + DecodeWithMemTracking + Debug + Sync + Send + Clone + Eq + PartialEq + StaticTypeInfo
{
	...
	fn implicit(&self) -> Result<Self::Implicit, TransactionValidityError> {
		use crate::runtime::transaction_validity::InvalidTransaction::IndeterminateImplicit;
...
macro_rules! impl_tx_ext_default {
	...
	($call:ty ; validate $( $rest:tt )*) => {
		fn validate(
			&self,
			origin: $crate::runtime::traits::DispatchOriginOf<$call>,
			_call: &$call,
			_info: &$crate::runtime::traits::DispatchInfoOf<$call>,
			_len: usize,
			_self_implicit: Self::Implicit,
			_inherited_implication: &impl $crate::runtime::codec::Encode,
			_source: $crate::runtime::transaction_validity::TransactionSource,
		) -> $crate::runtime::traits::ValidateResult<Self::Val, $call> {
	...
	($call:ty ; prepare $( $rest:tt )*) => {
		fn prepare(
			self,
			_val: Self::Val,
			_origin: &$crate::runtime::traits::DispatchOriginOf<$call>,
			_call: &$call,
			_info: &$crate::runtime::traits::DispatchInfoOf<$call>,
			_len: usize,
		) -> Result<Self::Pre, $crate::runtime::transaction_validity::TransactionValidityError> {
			Ok(Default::default())
		}
		$crate::impl_tx_ext_default!{$call ; $( $rest )*}
	};
	($call:ty ; weight $( $rest:tt )*) => {
		fn weight(&self, _call: &$call) -> $crate::runtime::Weight {
			$crate::runtime::Weight::zero()
		}
		$crate::impl_tx_ext_default!{$call ; $( $rest )*}
	};
	($call:ty ;) => {};
