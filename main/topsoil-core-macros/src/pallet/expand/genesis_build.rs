...
pub fn expand_genesis_build(def: &mut Def) -> proc_macro2::TokenStream {
	...
	quote::quote_spanned!(genesis_build.attr_span =>
		#topsoil_core::std_enabled! {
			impl<#type_impl_gen> #topsoil_core::subsoil::runtime::BuildStorage for #gen_cfg_ident<#gen_cfg_use_gen> #where_clause
			{
				fn assimilate_storage(&self, storage: &mut #topsoil_core::subsoil::runtime::Storage) -> std::result::Result<(), std::string::String> {
					#topsoil_core::__private::BasicExternalities::execute_with_storage(storage, || {
						self.build();
						Ok(())
					})
				}
