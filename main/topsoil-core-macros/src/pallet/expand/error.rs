...
pub fn expand_error(def: &mut Def) -> proc_macro2::TokenStream {
	...
	quote::quote_spanned!(error.attr_span =>
		...
		impl<#type_impl_gen> From<#error_ident<#type_use_gen>>
			for #topsoil_core::subsoil::runtime::DispatchError
			#config_where_clause
		{
			fn from(err: #error_ident<#type_use_gen>) -> Self {
				...
				encoded.resize(#topsoil_core::MAX_MODULE_ERROR_ENCODED_SIZE, 0);

				#topsoil_core::subsoil::runtime::DispatchError::Module(#topsoil_core::subsoil::runtime::ModuleError {
					index,
		...
		impl<#type_impl_gen> #error_ident<#type_use_gen> #config_where_clause {
			#[allow(dead_code)]
			#[doc(hidden)]
			pub fn error_metadata() -> #topsoil_core::__private::metadata_ir::PalletErrorMetadataIR {
				#(#maybe_allow_attrs)*
				#topsoil_core::__private::metadata_ir::PalletErrorMetadataIR {
					ty: #topsoil_core::__private::scale_info::meta_type::<#error_ident<#type_use_gen>>(),
