...
fn compute_disambiguation_path(
	...
	match (disambiguation_path, foreign_impl.clone().trait_) {
		...
		(None, Some((_, foreign_impl_path, _))) => {
			if default_impl_path.segments.len() > 2 {
				// Use all segments except the last two (sub-module + type name) as the scope.
				// e.g. topsoil_core::system::config_preludes::TestDefaultConfig
				//   → scope = topsoil_core::system
				//   → disambiguation = topsoil_core::system::DefaultConfig
				let scope_segments: syn::punctuated::Punctuated<_, syn::Token![::]> =
					default_impl_path
						.segments
						.iter()
						.take(default_impl_path.segments.len() - 2)
						.cloned()
						.collect();
				let scope = syn::Path {
					leading_colon: default_impl_path.leading_colon,
					segments: scope_segments,
				};
				Ok(parse_quote!(#scope :: #foreign_impl_path))
			} else if default_impl_path.segments.len() > 1 {
...
fn test_derive_impl_attr_args_parsing() {
	...
	.unwrap();
	parse2::<DeriveImplAttrArgs>(quote!(
		topsoil_core::system::prelude::testing::TestDefaultConfig as DefaultConfig
	))
	.unwrap();
