...
pub(crate) mod columns {
	pub const META: u32 = super::utils::COLUMN_META;
...
impl self::state_db::MetaDb for StateMetaDb {
								...
								match self.db.get(columns::TRANSACTION, hash.as_ref()) {
									Some(t) => {
										...
										let ex = Block::Extrinsic::decode(&mut input).map_err(
											|err| {
												crate::blockchain::Error::Backend(format!(
...
impl<Block: BlockT> crate::client_api::blockchain::HeaderBackend<Block> for BlockchainDb<Block> {
	...
	fn info(&self) -> crate::client_api::blockchain::Info<Block> {
		let meta = self.meta.read();
		crate::client_api::blockchain::Info {
...
impl<Block: BlockT> crate::client_api::blockchain::Backend<Block> for BlockchainDb<Block> {
	...
	fn block_indexed_body(&self, hash: Block::Hash) -> ClientResult<Option<Vec<Vec<u8>>>> {
		...
		match Vec::<DbExtrinsic<Block>>::decode(&mut &body[..]) {
			...
			Err(err) => {
				Err(crate::blockchain::Error::Backend(format!("Error decoding body list: {err}")))
			},
		}
	}
}
...
impl<Block: BlockT> HeaderMetadata<Block> for BlockchainDb<Block> {
	type Error = crate::blockchain::Error;
...
impl<Block: BlockT> BlockImportOperation<Block> {
	fn apply_offchain(&mut self, transaction: &mut Transaction<DbHash>) {
		...
		for ((prefix, key), value_operation) in self.offchain_storage_updates.drain(..) {
			count += 1;
			let key = self::offchain::concatenate_prefix_and_key(&prefix, &key);
	...
	fn apply_new_state(
		...
		if storage.top.keys().any(|k| well_known_keys::is_child_storage_key(k)) {
			return Err(crate::blockchain::Error::InvalidState);
		}
...
impl<Block: BlockT> crate::client_api::backend::BlockImportOperation<Block>
...
impl<Block: BlockT> self::state_db::NodeDb for StorageDb<Block> {
	type Error = io::Error;
	type Key = [u8];
...
impl<Block: BlockT> Backend<Block> {
	...
	pub fn new(db_config: DatabaseSettings, canonicalization_delay: u64) -> ClientResult<Self> {
		...
		let (needs_init, db) =
			match self::utils::open_database::<Block>(db_source, DatabaseType::Full, false) {
				Ok(db) => (false, db),
				Err(OpenDbError::DoesNotExist) => {
					let db =
						self::utils::open_database::<Block>(db_source, DatabaseType::Full, true)?;
					(true, db)
				},
	...
	pub fn new_test_with_tx_storage_and_filters(
		...
		let db = kvdb_memorydb::create(self::utils::NUM_COLUMNS);
	...
	fn from_database(
		...
		let map_e = crate::blockchain::Error::from_state_db;
		...
		if info.finalized_state.is_none()
			...
			&& crate::client_api::Backend::have_state_at(
				&backend,
				info.finalized_hash,
		...
		//
		// This leads to inefficient syncing and high CPU usage on every restart. To mitigate
		// this, remove the gap from the DB if we detect it and the current node is not an
		// archive.
	...
	fn set_head_with_transaction(
		...
		{
			return Err(crate::blockchain::Error::SetHeadTooOld);
		}

		let parent_exists =
			self.blockchain.status(route_to)? == crate::blockchain::BlockStatus::InChain;

		// Cannot find tree route with empty DB or when imported a detached block.
		if meta.best_hash != Default::default() && parent_exists {
			let tree_route =
				crate::blockchain::tree_route(&self.blockchain, meta.best_hash, route_to)?;
			...
			for r in tree_route.retracted() {
				if r.hash == meta.finalized_hash {
					...
					return Err(crate::blockchain::Error::NotInFinalizedChain);
				}
	...
	fn ensure_sequential_finalization(
		...
		{
			return Err(crate::blockchain::Error::NonSequentialFinalization(format!(
	...
	fn finalize_block_with_transaction(
		...
		let with_state = crate::client_api::Backend::have_state_at(self, hash, number);
	...
	fn force_delayed_canonicalize(
		...
		{
			let hash_to_canonicalize = crate::client_api::blockchain::HeaderBackend::hash(
				&self.blockchain,
			...
			if !crate::client_api::Backend::have_state_at(
				self,
			...
			let commit = self.storage.state_db.canonicalize_block(&hash_to_canonicalize).map_err(
				crate::blockchain::Error::from_state_db::<
					self::state_db::Error<subsoil::database::error::DatabaseError>,
				>,
			)?;
	...
	fn try_commit_operation(&self, mut operation: BlockImportOperation<Block>) -> ClientResult<()> {
		...
		let imported = if let Some(pending_block) = operation.pending_block {
			...
			let finalized = if operation.commit_state {
				let mut changeset: self::state_db::ChangeSet<Vec<u8>> =
					self::state_db::ChangeSet::default();
				let mut ops: u64 = 0;
				let mut bytes: u64 = 0;
		...
		if let Some(set_head) = operation.set_head {
			if let Some(header) =
				crate::client_api::blockchain::HeaderBackend::header(&self.blockchain, set_head)?
			{
				let number = header.number();
				...
				return Err(crate::blockchain::Error::UnknownBlock(format!(
...
fn apply_state_commit(
	transaction: &mut Transaction<DbHash>,
	commit: self::state_db::CommitSet<Vec<u8>>,
) {
...
impl<Block> crate::client_api::backend::AuxStore for Backend<Block>
where
	Block: BlockT,
{
	fn insert_aux<
...
impl<Block: BlockT> crate::client_api::backend::Backend<Block> for Backend<Block> {
	...
	fn commit_operation(&self, operation: Self::BlockImportOperation) -> ClientResult<()> {
		...
		if let Err(e) = self.try_commit_operation(operation) {
			...
			self.storage
				.state_db
				.reset(state_meta_db)
				.map_err(crate::blockchain::Error::from_state_db)?;
		...
		let mut revert_blocks = || -> ClientResult<NumberFor<Block>> {
			for c in 0..n.saturated_into::<u64>() {
				...
				let removed = self.blockchain.header(hash_to_revert)?.ok_or_else(|| {
					crate::blockchain::Error::UnknownBlock(format!(
	...
	fn state_at(
		...
		match self.blockchain.header_metadata(hash) {
			Ok(ref hdr) => {
				let hint = || {
					self::state_db::NodeDb::get(self.storage.as_ref(), hdr.state_root.as_ref())
						.unwrap_or(None)
						.is_some()
				};
					...
					self.storage.state_db.pin(&hash, hdr.number.saturated_into::<u64>(), hint)
				{
					...
					Err(crate::blockchain::Error::UnknownBlock(format!(
						"State already discarded for {hash:?}",
					)))
				}
			},
	...
	fn pin_block(&self, hash: <Block as BlockT>::Hash) -> crate::blockchain::Result<()> {
		let hint = || {
...
impl<Block: BlockT> crate::client_api::backend::LocalBackend<Block> for Backend<Block> {}
...
pub(crate) mod tests {
	...
	) -> Result<H256, crate::blockchain::Error> {
	...
	fn test_finalize_block_with_justification() {
		use crate::client_api::blockchain::Backend as BlockChainBackend;
	...
	fn test_import_existing_state_fails() {
		...
		let err = insert_block(&backend, 1, genesis, None, Default::default(), vec![], None)
			...
			.unwrap();
		match err {
			crate::blockchain::Error::StateDatabase(m) if m == "Block already exists" => (),
	...
	fn revert_non_best_blocks() {
		...
		let ensure_pruned = |hash, number: u32| {
			assert_eq!(
				...
				crate::client_api::blockchain::BlockStatus::Unknown
			);
			assert!(
				backend
					.blockchain
