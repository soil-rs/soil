...
- **Consensus engines stay separate.** A consumer picks one or two — don't merge them together.
...
### `soil-consensus` — Consensus framework (~4 crates → 1)

Base consensus traits and shared helpers. Not per-engine code.
...
### `soil-consensus-{engine}` — Per-engine crates (kept separate)

Each engine is an independent choice. Don't merge between engines.

- `soil-consensus-aura`
- `soil-consensus-babe` (absorbs its `-rpc`)
- `soil-consensus-grandpa` (absorbs its `-rpc`)
- `soil-consensus-beefy` (absorbs its `-rpc`)
- `soil-consensus-sassafras`
- `soil-consensus-pow`
...
| **soil-consensus** | base consensus + slots + epochs + block-builder | ~4 |
| **soil-consensus-{engine}** | kept separate per engine | ~7 |
...
**152 crates → ~20.** All 5 circular dependency clusters become crate-internal.
